// ==========================================================
// ===== LPXCONSTRUTOR - COMPLETO FINAL (TODAS CORREÇÕES) =====
// ==========================================================
    ...
    function configurar() {
        ...
        if (nav) {
            ...
            for (var j = 0; j < botoesNav.length; j++) {
                ...
                if (txt.indexOf('chat') >= 0 || txt === '💬') { btn.onclick = function(e) { e.preventDefault(); e.stopPropagation(); window.app.carregarListaConversas(); }; btn.style.cursor = 'pointer'; }
...
App.prototype.mostrarTela = function(id) {
    ...
    if (id === 'chatScreen') {
        if (s.usuarioSelecionado) { s.carregarMensagens(); }
        else { s.carregarListaConversas(); }
    }
    if (id === 'configScreen') s.carregarConfigScreen();
};

// VOLTAR NUNCA FECHA O APP
...
// ===== CHAT (LISTA DE CONVERSAS + CONVERSA INDIVIDUAL) =====
...
App.prototype.buscarConversas = function() {
    var s = this, c = document.getElementById('listaConversas');
    if (!c) return;
    
    if (typeof db !== 'undefined') {
        db.collection('mensagens').where('participantes', 'array-contains', s.usuarioLogado.id).orderBy('dataEnvio', 'desc').get().then(function(snap) {
            var conversas = {};
            snap.forEach(function(doc) {
                var m = doc.data();
                var outroId = m.remetenteId === s.usuarioLogado.id ? m.destinatarioId : m.remetenteId;
                if (!conversas[outroId]) {
                    conversas[outroId] = { id: outroId, ultimaMensagem: m.conteudo, data: m.dataEnvio ? (m.dataEnvio.toDate ? m.dataEnvio.toDate() : new Date(m.dataEnvio)) : new Date(), naoLidas: 0 };
                }
                if (m.destinatarioId === s.usuarioLogado.id && !m.lida) conversas[outroId].naoLidas++;
            });
            
            // Buscar conexões sem mensagem
            if (typeof databaseService !== 'undefined') {
                databaseService.buscarConexoes(s.usuarioLogado.id).then(function(conexoes) {
                    var promessas = [];
                    for (var i = 0; i < conexoes.length; i++) {
                        var amigoId = conexoes[i].usuarioId === s.usuarioLogado.id ? conexoes[i].amigoId : conexoes[i].usuarioId;
                        if (!conversas[amigoId]) {
                            promessas.push(databaseService.buscarUsuario(amigoId));
                        }
                    }
                    
                    // Buscar nomes para todas as conversas
                    var ids = Object.keys(conversas);
                    for (var j = 0; j < ids.length; j++) {
                        promessas.push(databaseService.buscarUsuario(ids[j]));
                    }
                    
                    Promise.all(promessas).then(function(usuarios) {
                        for (var k = 0; k < usuarios.length; k++) {
                            if (usuarios[k] && conversas[usuarios[k].id]) {
                                conversas[usuarios[k].id].nome = usuarios[k].nome;
                                conversas[usuarios[k].id].foto = usuarios[k].fotoPerfil;
                                conversas[usuarios[k].id].profissao = usuarios[k].profissao;
                            }
                        }
                        s.renderizarListaConversas(conversas, c);
                    }).catch(function() {
                        s.renderizarListaConversas(conversas, c);
                    });
                }).catch(function() {
            ...
            c.innerHTML = '<div style="text-align:center;padding:30px;color:#666;">Erro ao carregar</div>';
        });
    } else {
        c.innerHTML = '<div style="text-align:center;padding:40px;"><div style="font-size:50px;">💬</div><h3>Nenhuma conversa</h3></div>';
    }
};
...
App.prototype.renderizarListaConversas = function(conversas, container) {
    var s = this;
    var ids = Object.keys(conversas);
    ids.sort(function(a, b) { return (conversas[b].data || new Date(0)) - (conversas[a].data || new Date(0)); });
    
    if (ids.length === 0) {
        container.innerHTML = '<div style="text-align:center;padding:40px;"><div style="font-size:50px;">💬</div><h3>Nenhuma conversa</h3><p style="color:#666;">Conecte-se com profissionais para conversar</p><button onclick="window.app.mostrarTela(\'buscaScreen\')" style="background:#1A3A5C;color:white;border:none;padding:10px 20px;border-radius:20px;cursor:pointer;margin-top:10px;">🔍 Buscar</button></div>';
        return;
    }
    ...
    for (var i = 0; i < ids.length; i++) {
        ...
        var nome = conv.nome || 'Usuário';
        var ultimaMsg = conv.ultimaMensagem || 'Nova conversa';
        var naoLidas = conv.naoLidas || 0;
        
        html += '<div onclick="window.app.abrirConversa(\'' + conv.id + '\')" style="background:white;border-radius:10px;padding:12px;margin-bottom:8px;display:flex;align-items:center;gap:10px;cursor:pointer;' + (naoLidas > 0 ? 'border-left:4px solid #10B981;' : '') + '">';
        html += '<div style="width:50px;height:50px;border-radius:50%;overflow:hidden;border:2px solid #1A3A5C;flex-shrink:0;">';
        html += conv.foto ? '<img src="' + conv.foto + '" style="width:100%;height:100%;object-fit:cover;">' : '<div style="width:100%;height:100%;background:#e5e7eb;display:flex;align-items:center;justify-content:center;font-size:22px;">👷</div>';
        html += '</div><div style="flex:1;min-width:0;"><div style="display:flex;justify-content:space-between;"><strong>' + nome + '</strong><span style="font-size:11px;color:#999;">' + (conv.data ? new Date(conv.data).toLocaleDateString('pt-BR') : '') + '</span></div><div style="font-size:13px;color:#666;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">' + ultimaMsg + '</div></div>';
        if (naoLidas > 0) html += '<span style="background:#10B981;color:white;border-radius:50%;min-width:24px;height:24px;display:flex;align-items:center;justify-content:center;font-size:12px;font-weight:bold;">' + naoLidas + '</span>';
        html += '</div>';
    }
...
App.prototype.abrirConversa = function(uid) {
    ...
    if (user) { s.usuarioSelecionado = user; s.abrirChat(user); s.iniciarListenerMensagens(); return; }
    
    if (typeof databaseService !== 'undefined') {
        databaseService.buscarUsuario(uid).then(function(userData) {
            if (userData) { s.usuarioSelecionado = userData; s.abrirChat(userData); s.iniciarListenerMensagens(); }
        });
    }
};
...
App.prototype.iniciarListenerMensagens = function() {
    ...
    if (typeof db !== 'undefined' && s.usuarioSelecionado) {
        s._listenerChat = db.collection('mensagens').where('participantes', 'array-contains', s.usuarioLogado.id).orderBy('dataEnvio', 'asc').onSnapshot(function(snapshot) {
            var mensagens = [];
            snapshot.forEach(function(doc) {
                var m = doc.data(); m.id = doc.id;
                if (m.participantes && m.participantes.indexOf(s.usuarioLogado.id) >= 0 && m.participantes.indexOf(s.usuarioSelecionado.id) >= 0) mensagens.push(m);
            });
            
            s.renderizarMensagens(mensagens);
            
            var novasRecebidas = mensagens.filter(function(m) { return m.remetenteId !== s.usuarioLogado.id && !m.lida; });
            if (novasRecebidas.length > 0 && s.telaAtual !== 'chatScreen') {
                s.adicionarNotificacao(s.usuarioLogado.id, '💬 ' + s.usuarioSelecionado.nome, novasRecebidas[novasRecebidas.length - 1].conteudo.substring(0, 50));
                s.mostrarToast('💬 Nova mensagem de ' + s.usuarioSelecionado.nome + '!', 'info');
            }
            if (novasRecebidas.length > 0 && s.telaAtual === 'chatScreen') {
                for (var i = 0; i < novasRecebidas.length; i++) { db.collection('mensagens').doc(novasRecebidas[i].id).update({ lida: true }).catch(function(){}); }
            }
        });
    }
};
...
App.prototype.renderizarMensagens = function(mensagens) {
    var c = document.getElementById('chatMensagens'); if (!c) return;
    if (!mensagens || mensagens.length === 0) { c.innerHTML = '<div style="text-align:center;padding:30px;color:#666;">Diga olá! 👋</div>'; return; }
    var html = '';
    for (var i = 0; i < mensagens.length; i++) {
        var m = mensagens[i], ehMeu = m.remetenteId === this.usuarioLogado.id, data = '';
        if (m.dataEnvio) { if (m.dataEnvio.toDate) data = m.dataEnvio.toDate().toLocaleTimeString('pt-BR', {hour: '2-digit', minute: '2-digit'}); else data = new Date(m.dataEnvio).toLocaleTimeString('pt-BR', {hour: '2-digit', minute: '2-digit'}); }
        html += '<div style="display:flex;justify-content:' + (ehMeu ? 'flex-end' : 'flex-start') + ';margin-bottom:8px;"><div style="max-width:75%;padding:10px 14px;border-radius:18px;' + (ehMeu ? 'background:#1A3A5C;color:white;border-bottom-right-radius:4px;' : 'background:white;color:#333;border-bottom-left-radius:4px;box-shadow:0 1px 2px rgba(0,0,0,0.1);') + '"><div style="font-size:14px;">' + (m.conteudo || '') + '</div><div style="font-size:10px;opacity:0.7;text-align:right;">' + data + (ehMeu ? ' ✓' : '') + '</div></div></div>';
    }
    c.innerHTML = html; c.scrollTop = c.scrollHeight;
};

App.prototype.carregarMensagens = function() { var s = this, c = document.getElementById('chatMensagens'); if (!c || !s.usuarioSelecionado) return; if (typeof db !== 'undefined') { c.innerHTML = '<div style="text-align:center;padding:30px;color:#666;">Carregando...</div>'; return; } };
...
App.prototype.enviarMensagem = function() {
    ...
    if (typeof db !== 'undefined') {
        db.collection('mensagens').add({ remetenteId: s.usuarioLogado.id, destinatarioId: s.usuarioSelecionado.id, participantes: [s.usuarioLogado.id, s.usuarioSelecionado.id], conteudo: texto, lida: false, dataEnvio: firebase.firestore.FieldValue.serverTimestamp() }).then(function() {
            input.value = ''; input.focus(); s._enviandoMensagem = false; if (btnEnviar) { btnEnviar.disabled = false; btnEnviar.textContent = 'Enviar'; }
            s.adicionarNotificacao(s.usuarioSelecionado.id, '💬 ' + s.usuarioLogado.nome, texto.substring(0, 50));
        }).catch(function() { s._enviandoMensagem = false; if (btnEnviar) { btnEnviar.disabled = false; btnEnviar.textContent = 'Enviar'; } });
    }
};
...
App.prototype.iniciarNotificacoesAutomaticas = function() { var s = this; if (!s.usuarioLogado) return; function verificar() { var hora = new Date().getHours(), dia = new Date().getDay(); if (dia >= 1 && dia <= 5) { if (hora >= 6 && hora < 8) s.adicionarNotificacao(s.usuarioLogado.id, '☀️ Bom dia!', 'Tenha um ótimo dia de trabalho, ' + s.usuarioLogado.nome + '! 💪🏗️'); if (hora >= 18 && hora < 20) s.adicionarNotificacao(s.usuarioLogado.id, '🌙 Fim do expediente', 'Bom descanso, ' + s.usuarioLogado.nome + '! 😴'); } s.verificarLocalizacaoObra(); } verificar(); if (s._intervaloNotificacoes) clearInterval(s._intervaloNotificacoes); s._intervaloNotificacoes = setInterval(verificar, 1800000); };

// ===== GEOFENCING =====
App.prototype.iniciarMonitoramentoObra = function(vagaId) { var s = this; if (!s.usuarioLogado) return; if (typeof db !== 'undefined') { db.collection('vagas').doc(vagaId).get().then(function(doc) { if (!doc.exists) return; var vaga = doc.data(); if (!vaga.lat || !vaga.lng) { s.mostrarToast('⚠️ Obra sem localização', 'erro'); return; } s.obraMonitorada = { vagaId: vagaId, lat: vaga.lat, lng: vaga.lng, titulo: vaga.titulo, autorId: vaga.autorId || vaga.usuarioId }; localStorage.setItem('obraMonitorada', JSON.stringify(s.obraMonitorada)); s.verificarLocalizacaoObra(); s.mostrarToast('📍 Monitoramento iniciado!', 'sucesso'); }); } };
App.prototype.pararMonitoramentoObra = function() { localStorage.removeItem('obraMonitorada'); this.obraMonitorada = null; if (this._intervaloLocalizacao) { clearInterval(this._intervaloLocalizacao); this._intervaloLocalizacao = null; } };
App.prototype.verificarLocalizacaoObra = function() { var s = this; if (!s.obraMonitorada) { var salvo = localStorage.getItem('obraMonitorada'); if (salvo) { try { s.obraMonitorada = JSON.parse(salvo); } catch(e) {} } } if (!s.obraMonitorada || !navigator.geolocation) return; navigator.geolocation.getCurrentPosition(function(pos) { var distancia = s.calcularDistancia(pos.coords.latitude, pos.coords.longitude, s.obraMonitorada.lat, s.obraMonitorada.lng); if (distancia > 100) { var ultimaAfast = localStorage.getItem('ultimaNotifAfastamento'); if (!ultimaAfast || (Date.now() - parseInt(ultimaAfast)) > 600000) { localStorage.setItem('ultimaNotifAfastamento', Date.now().toString()); s.adicionarNotificacao(s.obraMonitorada.autorId, '⚠️ Profissional se afastou!', s.usuarioLogado.nome + ' se afastou ' + distancia.toFixed(0) + 'm da obra'); s.mostrarToast('⚠️ Você se afastou da obra!', 'erro'); } } if (distancia < 50) { var ultimaChegada = localStorage.getItem('ultimaNotifChegada'); if (!ultimaChegada || (Date.now() - parseInt(ultimaChegada)) > 3600000) { localStorage.setItem('ultimaNotifChegada', Date.now().toString()); s.adicionarNotificacao(s.usuarioLogado.id, '📍 Chegou na obra!', 'Bom trabalho em: ' + s.obraMonitorada.titulo + ' 💪'); s.mostrarToast('📍 Chegou na obra! 💪', 'sucesso'); } } }, function() {}, { enableHighAccuracy: true }); };
App.prototype.calcularDistancia = function(lat1, lon1, lat2, lon2) { var R = 6371000, dLat = (lat2 - lat1) * Math.PI / 180, dLon = (lon2 - lon1) * Math.PI / 180; var a = Math.sin(dLat/2) * Math.sin(dLat/2) + Math.cos(lat1 * Math.PI / 180) * Math.cos(lat2 * Math.PI / 180) * Math.sin(dLon/2) * Math.sin(dLon/2); return R * 2 * Math.atan2(Math.sqrt(a), Math.sqrt(1-a)); };
...
// ===== AVALIAÇÃO =====
App.prototype.avaliarUsuario = function(avaliadoId) { var s = this; if (!s.usuarioLogado) return; var nota = prompt('Dê uma nota de 1 a 5 ⭐:'); if (!nota || isNaN(nota) || nota < 1 || nota > 5) { s.mostrarToast('❌ Nota inválida! (1-5)', 'erro'); return; } if (typeof db !== 'undefined') { db.collection('avaliacoes').add({ avaliadorId: s.usuarioLogado.id, avaliadoId: avaliadoId, nota: parseInt(nota), dataCriacao: firebase.firestore.FieldValue.serverTimestamp() }).then(function() { db.collection('avaliacoes').where('avaliadoId', '==', avaliadoId).get().then(function(snap) { var total = 0, soma = 0; snap.forEach(function(doc) { total++; soma += doc.data().nota; }); var score = total > 0 ? Math.round((soma / total) * 10) / 10 : 0; db.collection('usuarios').doc(avaliadoId).update({ score: score, avaliacoesRecebidas: total }).catch(function(){}); }); s.mostrarToast('⭐ Avaliação enviada!', 'sucesso'); }); } };
...
App.prototype.mostrarDetalheObraModal = function(v) { var s = this; var autorId = v.autorId || v.usuarioId; var html = '<div id="modalObra" style="position:fixed;top:0;left:0;right:0;bottom:0;background:rgba(0,0,0,0.5);z-index:9999;overflow-y:auto;" onclick="if(event.target===this)this.remove()"><div style="background:white;min-height:100vh;max-width:500px;margin:0 auto;">' + (v.fotoObra && v.fotoObra.length > 100 ? '<img src="' + v.fotoObra + '" style="width:100%;max-height:300px;object-fit:cover;">' : '') + '<div style="padding:20px;"><h2>' + (v.titulo || 'Sem título') + '</h2><div style="background:#f9fafb;border-radius:12px;padding:15px;margin-bottom:15px;"><p>📍 ' + (v.endereco || '') + '</p><p>👷 ' + (v.profissoes || 'Todas') + '</p><p>💰 R$' + (v.valorHora || '0') + '/h</p></div>'; if (v.endereco) html += '<a href="https://www.google.com/maps?q=' + encodeURIComponent(v.endereco) + '" target="_blank" style="display:block;text-align:center;background:#1A3A5C;color:white;padding:12px;border-radius:10px;text-decoration:none;font-weight:bold;margin-bottom:15px;">🗺️ Google Maps</a>'; html += '<div style="display:flex;flex-direction:column;gap:10px;">'; if (s.usuarioLogado && s.usuarioLogado.tipo === 'profissional' && autorId !== s.usuarioLogado.id) html += '<button onclick="window.app.candidatarVaga(\'' + v.id + '\');document.getElementById(\'modalObra\').remove();" style="width:100%;background:#10B981;color:white;border:none;padding:15px;border-radius:10px;font-weight:bold;cursor:pointer;">✋ CANDIDATAR-SE</button>'; if (s.usuarioLogado && s.usuarioLogado.id === autorId) html += '<button onclick="window.app.verCandidatos(\'' + v.id + '\');document.getElementById(\'modalObra\').remove();" style="width:100%;background:#f59e0b;color:white;border:none;padding:15px;border-radius:10px;font-weight:bold;cursor:pointer;">👷 VER CANDIDATOS</button>'; if (s.usuarioLogado && autorId && autorId !== s.usuarioLogado.id) html += '<button onclick="window.app.iniciarChat(\'' + autorId + '\');document.getElementById(\'modalObra\').remove();" style="width:100%;background:#1A3A5C;color:white;border:none;padding:15px;border-radius:10px;font-weight:bold;cursor:pointer;">💬 CONVERSAR PELO APP</button>'; html += '<button onclick="window.app._app.contatarWhatsApp(\'' + autorId + '\')" style="width:100%;background:#25D366;color:white;border:none;padding:15px;border-radius:10px;font-weight:bold;cursor:pointer;">📱 WHATSAPP</button>'; html += '<button onclick="document.getElementById(\'modalObra\').remove()" style="width:100%;background:#6b7280;color:white;border:none;padding:15px;border-radius:10px;cursor:pointer;">⬅ Fechar</button></div></div></div></div>'; var antigo = document.getElementById('modalObra'); if (antigo) antigo.remove(); document.body.insertAdjacentHTML('beforeend', html); };
App.prototype.contatarWhatsApp = function(autorId) { var s = this; var usuarios = JSON.parse(localStorage.getItem('usuariosLPX') || '[]'); var autor = usuarios.find(function(u) { return u.id === autorId; }); if (autor && autor.celular) { window.open('https://wa.me/55' + autor.celular.replace(/\D/g, ''), '_blank'); } else if (typeof databaseService !== 'undefined') { databaseService.buscarUsuario(autorId).then(function(user) { if (user && user.celular) window.open('https://wa.me/55' + user.celular.replace(/\D/g, ''), '_blank'); else s.mostrarToast('📱 Celular não disponível', 'erro'); }); } else { s.mostrarToast('📱 Celular não disponível', 'erro'); } };
...
App.prototype.getTodasCidades = function() { return { 'SP':['São Paulo','Campinas','Santos'],'RJ':['Rio de Janeiro','Niterói'],'MG':['Belo Horizonte','Uberlândia'],'SC':['Florianópolis','Joinville','Blumenau'],'PR':['Curitiba','Londrina'],'RS':['Porto Alegre','Caxias do Sul'] }; };
App.prototype.getBairrosPorCidade = function(c) { var b = { 'São Paulo':['Centro','Pinheiros','Vila Mariana'],'Rio de Janeiro':['Copacabana','Ipanema','Barra'],'Florianópolis':['Centro','Lagoa','Ingleses'],'Joinville':['Centro','América','Glória'],'Curitiba':['Centro','Batel'] }; return b[c] || ['Centro']; };
...
App.prototype.inicializarMapaLocalizacao = function() { var md = document.getElementById('mapaLocalizacao'); if (!md) return; var lat = -23.5505, lng = -46.6333; if (navigator.geolocation) { navigator.geolocation.getCurrentPosition(function(p) { window.app._app.mostrarMapaPadrao(md, p.coords.latitude, p.coords.longitude); }, function() { window.app._app.mostrarMapaPadrao(md, lat, lng); }); } else { this.mostrarMapaPadrao(md, lat, lng); } };
App.prototype.mostrarMapaPadrao = function(md, lat, lng) { if (typeof google === 'undefined') { md.innerHTML = '<div style="display:flex;align-items:center;justify-content:center;height:100%;background:#f0f9ff;border-radius:12px;color:#666;">🗺️ Mapa indisponível</div>'; return; } var map = new google.maps.Map(md, { center: { lat: lat, lng: lng }, zoom: 15 }); var marker = new google.maps.Marker({ position: { lat: lat, lng: lng }, map: map, draggable: true }); google.maps.event.addListener(marker, 'dragend', function() { var p = marker.getPosition(); window.app._mapaMarcador = { lat: p.lat(), lng: p.lng() }; }); window.app._mapaMarcador = { lat: lat, lng: lng }; };
App.prototype.salvarLocalizacao = function() { var s = this; var est = document.getElementById('locEstado')?.value || '', cid = document.getElementById('locCidade')?.value || '', bai = document.getElementById('locBairroSelect')?.value || ''; if (!est || !cid) { s.mostrarToast('❌ Preencha estado e cidade!', 'erro'); return; } var loc = { estado: est, cidade: cid, bairro: bai, lat: window.app._mapaMarcador ? window.app._mapaMarcador.lat : -23.5505, lng: window.app._mapaMarcador ? window.app._mapaMarcador.lng : -46.6333 }; s.usuarioLogado.localizacao = loc; localStorage.setItem('usuarioLPX', JSON.stringify(s.usuarioLogado)); if (typeof databaseService !== 'undefined') databaseService.atualizarUsuario(s.usuarioLogado.id, { localizacao: loc }).catch(function(){}); document.getElementById('modalLocalizacao')?.remove(); s.mostrarToast('✅ Localização salva!', 'sucesso'); s.carregarMeuPerfil(); };
...
App.prototype.uploadFoto = function(event) { var s = this, file = event.target.files[0]; if (!file) return; var reader = new FileReader(); reader.onload = function(e) { var foto = e.target.result; s.usuarioLogado.fotoPerfil = foto; localStorage.setItem('usuarioLPX', JSON.stringify(s.usuarioLogado)); if (typeof databaseService !== 'undefined') databaseService.atualizarUsuario(s.usuarioLogado.id, { fotoPerfil: foto }).catch(function(){}); var preview = document.getElementById('fotoPerfilPreview'); if (preview) preview.src = foto; s.mostrarToast('✅ Foto atualizada!', 'sucesso'); }; reader.readAsDataURL(file); };
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    console.log('✅ Lista de conversas com histórico');
    console.log('✅ Mensagens em tempo real');
    console.log('✅ Candidatura e contratação');
    console.log('✅ Geofencing e notificações automáticas');
});
