...
App.prototype.init = function() {
    ...
    authService.onAuthStateChange(function(usuario) {
        if (usuario) { self.usuarioLogado = usuario; self.atualizarBotaoPublicar(); if (self.telaAtual === 'loginScreen' || self.telaAtual === 'cadastroScreen') self.mostrarTela('homeScreen'); }
        else { self.usuarioLogado = null; self.mostrarTela('loginScreen'); }
    });
};
...
App.prototype.mostrarTela = function(id) {
    ...
    if (id === 'recuperarSenhaScreen') { document.getElementById('recPasso1').style.display = 'block'; document.getElementById('recPasso2').style.display = 'none'; var emailEl = document.getElementById('recEmail'); if (emailEl) emailEl.value = ''; setTimeout(function() { if (emailEl) emailEl.focus(); }, 500); }
    if (id === 'chatScreen') setTimeout(function() { var input = document.getElementById('chatInput'); if (input) input.focus(); }, 300);
};
...
App.prototype.solicitarCodigo = function() {
    ...
    var status1 = document.getElementById('recStatus1'), icon1 = document.getElementById('recIcon1'), msg1 = document.getElementById('recMsg1'), sub1 = document.getElementById('recSub1'), codMostrado = document.getElementById('recCodigoMostrado');
    if (status1) { status1.style.display = 'block'; status1.style.background = '#E0F2FE'; status1.style.border = '2px solid #7DD3FC'; }
    if (icon1) icon1.textContent = '⏳'; if (msg1) msg1.textContent = 'Gerando código...'; if (sub1) sub1.textContent = 'Aguarde'; if (codMostrado) codMostrado.style.display = 'none';
    var btn = document.querySelector('#recPasso1 .btn-primary'); if (btn) { btn.disabled = true; btn.style.opacity = '0.7'; btn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> GERANDO...'; }
    authService.solicitarCodigoRecuperacao(email).then(function(r) {
        ...
        if (r.sucesso) {
            ...
            if (icon1) icon1.textContent = '🔢'; if (msg1) { msg1.textContent = 'Código gerado!'; msg1.style.color = '#065F46'; }
            if (sub1) { sub1.textContent = 'Use o código abaixo:'; sub1.style.color = '#065F46'; }
            if (codMostrado) { codMostrado.style.display = 'block'; codMostrado.textContent = r.codigo; codMostrado.style.background = '#1A3A5C'; codMostrado.style.color = 'white'; codMostrado.style.padding = '20px'; codMostrado.style.borderRadius = '12px'; codMostrado.style.fontSize = '36px'; codMostrado.style.fontWeight = '900'; codMostrado.style.letterSpacing = '12px'; }
            ...
            if (icon1) icon1.textContent = '❌'; if (msg1) { msg1.textContent = r.erro; msg1.style.color = '#991B1B'; }
            self.mostrarToast('❌ ' + r.erro, 'erro'); if (btn) { btn.disabled = false; btn.style.opacity = '1'; btn.innerHTML = '<i class="fas fa-paper-plane"></i> TENTAR NOVAMENTE'; }
        }
    });
};
...
App.prototype.verificarCodigo = function() {
    ...
    if (!novaSenha || novaSenha.length < 6) { self.mostrarToast('❌ Senha mínima 6 caracteres!', 'erro'); return; }
    var status2 = document.getElementById('recStatus2'); if (status2) { status2.style.display = 'block'; status2.style.background = '#E0F2FE'; status2.style.border = '2px solid #7DD3FC'; status2.style.padding = '16px'; status2.style.borderRadius = '12px'; status2.innerHTML = '<p>⏳ Verificando...</p>'; }
    var btn = document.querySelector('#recPasso2 .btn-primary'); if (btn) { btn.disabled = true; btn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> VERIFICANDO...'; }
    authService.redefinirSenhaComCodigo(self.recuperacaoUid, codigo, novaSenha).then(function(r) {
        ...
        if (r.sucesso) { if (status2) { status2.style.background = '#D1FAE5'; status2.style.border = '2px solid #6EE7B7'; status2.innerHTML = '<div style="font-size:40px;">✅</div><p style="color:#065F46;">Código verificado!</p>'; } self.mostrarToast('✅ Verificado!', 'sucesso'); if (btn) { btn.innerHTML = '<i class="fas fa-check"></i> VERIFICADO!'; btn.style.background = '#10B981'; } setTimeout(function() { self.mostrarTela('loginScreen'); document.getElementById('recPasso1').style.display = 'block'; document.getElementById('recPasso2').style.display = 'none'; }, 3000); }
        else { if (status2) { status2.style.background = '#FEE2E2'; status2.style.border = '2px solid #FCA5A5'; status2.innerHTML = '<p style="color:#991B1B;">❌ ' + r.erro + '</p>'; } self.mostrarToast('❌ ' + r.erro, 'erro'); if (btn) { btn.disabled = false; btn.innerHTML = '<i class="fas fa-check"></i> REDEFINIR SENHA'; } }
    });
};
...
App.prototype.carregarHome = function() {
    ...
    var saudacaoEl = document.getElementById('saudacao'); if (saudacaoEl) saudacaoEl.textContent = '👋 ' + s + ', ' + this.usuarioLogado.nome + '!';
    var resumoEl = document.getElementById('resumoTexto'); if (resumoEl) resumoEl.textContent = (this.usuarioLogado.tipo === 'empreiteiro' ? '🏢 Empreiteiro' : '👷 Profissional') + ' • ' + (this.usuarioLogado.profissao || this.usuarioLogado.tipo);
...
App.prototype.contratarProfissional = function(profId) { var self = this; if (!this.usuarioLogado || this.usuarioLogado.tipo !== 'empreiteiro') { this.mostrarToast('Apenas empreiteiros!', 'erro'); return; } var obra = prompt('Nome da obra:'); if (!obra) return; var valor = prompt('Valor/hora (R$):', '25'); if (!valor) return; db.collection('conexoes').add({ usuarioId: this.usuarioLogado.id, amigoId: profId, status: 'contratado', obra: obra, valorHora: parseFloat(valor), dataCriacao: firebase.firestore.FieldValue.serverTimestamp() }).then(function() { self.mostrarToast('🤝 Contratado!', 'sucesso'); self.adicionarNaRede(profId); }); };

App.prototype.carregarMeuPerfil = function() { if (!this.usuarioLogado) return; var u = this.usuarioLogado; var n = document.getElementById('meuPerfilNome'); if (n) n.textContent = u.nome || 'Usuário'; var p = document.getElementById('meuPerfilProfissao'); if (p) p.textContent = (u.tipo === 'profissional' ? '👷' : '🏢') + ' ' + (u.profissao || u.tipo); var a = document.getElementById('meuPerfilAvaliacao'); if (a) a.innerHTML = '⭐'.repeat(Math.round(u.score || 0)) + ' ' + (u.score ? u.score.toFixed(1) : 'Sem avaliações'); var en = document.getElementById('editNome'); if (en) en.value = u.nome || ''; var ec = document.getElementById('editCelular'); if (ec) ec.value = u.celular || ''; var eh = document.getElementById('editHabilidades'); if (eh) eh.value = u.habilidades || ''; var sa = document.getElementById('statsAvaliacoes'); if (sa) sa.textContent = u.avaliacoesRecebidas || 0; };
...
App.prototype.verPerfil = function(uid) {
    ...
    db.collection('usuarios').doc(uid).get().then(function(doc) {
        ...
        if (self.usuarioLogado && self.usuarioLogado.id !== uid) {
            ...
            if (w) { var msgWhats = self.usuarioLogado.tipo === 'empreiteiro' ? 'Olá ' + u.nome.split(' ')[0] + '! Me chamo ' + self.usuarioLogado.nome + ', sou empreiteiro e vi seu perfil no LPXConstrutor. Tenho interesse no seu trabalho como ' + (u.profissao || 'profissional') + '. Podemos conversar?' : 'Olá ' + u.nome.split(' ')[0] + '! Vi seu perfil no LPXConstrutor. Vamos nos conectar?'; html += '<a href="https://wa.me/55' + w + '?text=' + encodeURIComponent(msgWhats) + '" target="_blank" class="btn btn-success"><i class="fab fa-whatsapp"></i> Chamar no WhatsApp</a>'; }
            html += '<button class="btn btn-primary" onclick="app.iniciarChat(\'' + uid + '\')">💬 Conversar no Chat</button>';
            html += '<button class="btn btn-outline" onclick="app.adicionarNaRede(\'' + uid + '\')" style="background:#10B981;color:white;">➕ Adicionar na Minha Rede</button>';
...
App.prototype.abrirTelaPublicacao = function() { if (!this.usuarioLogado || this.usuarioLogado.tipo !== 'empreiteiro') { this.mostrarToast('Apenas empreiteiros!', 'erro'); return; } ['vagaTitulo','vagaDescricao','vagaEndereco'].forEach(function(id) { var el = document.getElementById(id); if (el) el.value = ''; }); document.getElementById('vagaValorHora').value = ''; document.getElementById('vagaQuantidade').value = '1'; document.querySelectorAll('#profissoesCheckboxes input').forEach(function(cb) { cb.checked = false; }); this.mostrarTela('publicarVagaScreen'); };
App.prototype.publicarVagaApp = function() { var self = this, t = (document.getElementById('vagaTitulo') || {}).value || '', e = (document.getElementById('vagaEndereco') || {}).value || ''; if (!t || !e) { this.mostrarToast('Preencha título e endereço!', 'erro'); return; } var ps = []; document.querySelectorAll('#profissoesCheckboxes input:checked').forEach(function(cb) { ps.push(cb.value); }); this.mostrarToast('Publicando...', 'info'); db.collection('vagas').add({ titulo: t, descricao: (document.getElementById('vagaDescricao') || {}).value || '', endereco: e, profissoes: ps.join(', ') || 'Geral', valorHora: parseFloat((document.getElementById('vagaValorHora') || {}).value) || 0, quantidade: parseInt((document.getElementById('vagaQuantidade') || {}).value) || 1, usuarioId: this.usuarioLogado.id, interessados: [], dataCriacao: firebase.firestore.FieldValue.serverTimestamp(), ativa: true }).then(function() { self.mostrarToast('✅ Vaga publicada!', 'sucesso'); setTimeout(function() { self.mostrarTela('homeScreen'); self.carregarFeed(); }, 1000); }); };
...
App.prototype.avaliarUsuario = function(uid) { var n = prompt('Nota (1-5):'); if (!n || isNaN(n) || n < 1 || n > 5) return; var c = prompt('Comentário:') || ''; var self = this; databaseService.avaliarUsuario(this.usuarioLogado.id, uid, parseInt(n), c).then(function() { self.mostrarToast('✅ Avaliação enviada!', 'sucesso'); }); };
