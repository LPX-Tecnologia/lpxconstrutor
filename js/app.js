// ==========================================================
// ===== LPXCONSTRUTOR - 100% FIREBASE (MULTI-DISPOSITIVOS) =====
// ==========================================================
...
App.prototype.init = function() {
    var s = this;
    console.log('🚀 LPXCONSTRUTOR - 100% FIREBASE');
    window.app._app = s;
    ...
    splash.innerHTML = '<img src="imagem/logo-sem-fundo-lpxconstrutor.png" style="width:120px;"><p style="color:white;font-size:22px;font-weight:900;margin-top:16px;">LPXCONSTRUTOR</p>';
    ...
    if (typeof firebase !== 'undefined' && firebase.auth) {
        firebase.auth().onAuthStateChanged(function(user) {
            ...
            setTimeout(function() { splash.style.opacity = '0'; setTimeout(function() { if (splash.parentNode) splash.parentNode.removeChild(splash); }, 500); }, 1500);
        });
    } else {
        // Fallback localStorage
        var salvo = localStorage.getItem('usuarioLPX');
        if (salvo) { try { s.usuarioLogado = JSON.parse(salvo); } catch(e) {} }
        setTimeout(function() { splash.style.opacity = '0'; setTimeout(function() { if (splash.parentNode) splash.parentNode.removeChild(splash); if (s.usuarioLogado) s.mostrarTela('homeScreen'); else s.mostrarTela('loginScreen'); }, 500); }, 1500);
    }
};
...
App.prototype.carregarHome = function() {
    ...
    html += '<div style="flex:1;"><span>👋 ' + sd + ', <b>' + user.nome + '</b>!</span></div></div>';
    ...
    if (typeof db !== 'undefined') {
        ...
        s._listenerFeed = db.collection('vagas')
            ...
            .onSnapshot(function(snap) {
                ...
                c.innerHTML = '<div style="text-align:center;padding:30px;">Erro ao carregar</div>';
            });
    }
};
...
App.prototype.apagarObra = function(oid, event) {
    ...
    if (typeof db !== 'undefined') {
        db.collection('vagas').doc(oid).update({ ativa: false }).then(function() {
            window.app._app.mostrarToast('Obra apagada!', 'sucesso');
        });
    }
};

// ===== PUBLICAR (SALVA NO FIREBASE) =====
App.prototype.publicarVagaApp = function() {
    ...
    var profissoes = (document.getElementById('pubProfissoes')||{}).value||'Geral';
    ...
    var vaga = {
        titulo: titulo, endereco: endereco, profissoes: profissoes,
        valorHora: parseFloat(valor)||0, fotoObra: s.vagaFotoBase64||'',
    ...
    if (typeof db !== 'undefined') {
        db.collection('vagas').add(vaga).then(function() {
            s.limparFormPublicacao();
            s.mostrarToast('✅ Obra publicada!', 'sucesso');
            s.historicoTelas = []; s.mostrarTela('homeScreen');
        }).catch(function() {
...
App.prototype.carregarRede = function() {
    ...
    if (typeof db !== 'undefined') {
        db.collection('conexoes').where('participantes', 'array-contains', s.usuarioLogado.id)
            .onSnapshot(function(snap) {
                var conexoes = [];
        ...
        if (typeof db !== 'undefined' && amigoId) {
            db.collection('usuarios').doc(amigoId).get().then(function(doc) {
                ...
                if (doc.exists) {
                    ...
                    html += '<div style="flex:1;cursor:pointer;" onclick="window.app.verPerfil(\'' + amigo.id + '\')"><strong>' + amigo.nome + '</strong><br><small>' + (amigo.profissao || '') + '</small></div>';
...
App.prototype.adicionarNaRede = function(amigoId) {
    ...
    if (typeof db !== 'undefined') {
        db.collection('conexoes').add({
            participantes: [s.usuarioLogado.id, amigoId],
            dataCriacao: firebase.firestore.FieldValue.serverTimestamp()
        }).then(function() {
    ...
    if (typeof db !== 'undefined') {
        db.collection('conexoes').where('participantes', 'array-contains', s.usuarioLogado.id)
            .onSnapshot(function(snap) {
                ...
                if (ids.length === 0) {
                    container.innerHTML = '<div style="text-align:center;padding:40px;"><div style="font-size:50px;">💬</div><h3>Nenhuma conversa</h3><button onclick="window.app.mostrarTela(\'buscaScreen\')" style="background:#1A3A5C;color:white;border:none;padding:10px 20px;border-radius:20px;cursor:pointer;">🔍 Buscar</button></div>';
                    return;
                }
        ...
        s._listenerChat = db.collection('mensagens')
            ...
            .onSnapshot(function(snap) {
                ...
                snap.forEach(function(doc) {
                    ...
                    if (m.participantes.indexOf(s.usuarioSelecionado.id) >= 0) msgs.push(m);
                });
                var c = document.getElementById('chatMensagens');
                if (!c) return;
                if (msgs.length === 0) { c.innerHTML = '<div style="text-align:center;padding:30px;color:#666;">Diga olá! 👋</div>'; return; }
                var html = '';
                for (var i = 0; i < msgs.length; i++) {
                    var meu = msgs[i].remetenteId === s.usuarioLogado.id;
                    html += '<div style="display:flex;justify-content:' + (meu ? 'flex-end' : 'flex-start') + ';margin-bottom:8px;"><div style="max-width:75%;padding:10px 14px;border-radius:18px;' + (meu ? 'background:#1A3A5C;color:white;' : 'background:white;color:#333;') + '">' + msgs[i].conteudo + '</div></div>';
                }
...
App.prototype.enviarMensagem = function() {
    ...
    if (!inp || !s.usuarioSelecionado) return;
    ...
    if (typeof db !== 'undefined') {
        db.collection('mensagens').add({
            ...
            inp.value = ''; s._enviandoMensagem = false;
        });
    }
};
...
App.prototype.buscarProfissionais = function() {
    ...
    if (typeof db !== 'undefined') {
        db.collection('usuarios').get().then(function(snap) {
            var todos = [];
            snap.forEach(function(doc) { var u = doc.data(); u.id = doc.id; if (u.id !== s.usuarioLogado?.id) todos.push(u); });
            if (todos.length === 0) { c.innerHTML = '<div style="text-align:center;padding:30px;"><h3>Nenhum profissional</h3></div>'; return; }
            var html = '<div style="text-align:center;padding:10px;">👷 ' + todos.length + ' profissional(is)</div>';
...
App.prototype.verPerfil = function(uid) {
    ...
    if (typeof db !== 'undefined') {
        db.collection('usuarios').doc(uid).get().then(function(doc) {
            ...
            var u = doc.data(); u.id = doc.id;
            var t = document.getElementById('perfilPublicoScreen');
            if (!t) { t = document.createElement('div'); t.id = 'perfilPublicoScreen'; t.className = 'screen'; document.body.appendChild(t); }
...
App.prototype.carregarMinhasObras = function() {
    ...
    if (!c || !s.usuarioLogado) return;
    c.innerHTML = '<div style="text-align:center;padding:30px;">Carregando...</div>';
    if (typeof db !== 'undefined') {
        db.collection('vagas').where('autorId', '==', s.usuarioLogado.id).where('ativa', '==', true).get().then(function(snap) {
            var minhas = []; snap.forEach(function(doc) { var v = doc.data(); v.id = doc.id; minhas.push(v); });
...
App.prototype.mostrarNotificacoes = function() { this.mostrarToast('🔔 Em breve!','info'); };
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    console.log('✅ LPXCONSTRUTOR - 100% FIREBASE');
    console.log('🔥 Todos os dados vêm do banco de dados');
    console.log('📱 Múltiplos dispositivos sincronizados');
});
