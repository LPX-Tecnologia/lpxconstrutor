...
App.prototype.voltarTela = function() {
    // NUNCA FECHA O APP - sempre volta para home se não tiver histórico
...
// ===== FEED (MOSTRA TODAS AS OBRAS PARA TODOS) =====
App.prototype.carregarFeed = function() {
    ...
    console.log('📊 Feed - Total de vagas:', vagas.length);
    if (vagas.length === 0) { c.innerHTML = '<div style="text-align:center;padding:30px;background:white;border-radius:10px;"><div style="font-size:50px;">🏗️</div><h3>Nenhuma obra publicada</h3>' + (s.usuarioLogado && s.usuarioLogado.tipo === 'empreiteiro' ? '<button onclick="window.app.abrirTelaPublicacao()" style="background:#f59e0b;color:white;border:none;padding:10px 20px;border-radius:20px;cursor:pointer;margin-top:10px;">📢 PUBLICAR</button>' : '') + '</div>'; return; }
    var html = '';
...
// ===== LOCALIZAÇÃO COMPLETA (TODAS CIDADES E BAIRROS) =====
App.prototype.abrirMapaLocalizacao = function() {
    ...
    h += '</div></div>';
    var a = document.getElementById('modalLoc'); if (a) a.remove();
...
App.prototype.getEstadosHTML = function(sel) {
    ...
    var h = ''; for (var s in e) h += '<option value="' + s + '"' + (sel === s ? ' selected' : '') + '>' + e[s] + ' (' + s + ')</option>';
    return h;
};

App.prototype.getTodasCidades = function() {
    return {
        'SP': ['São Paulo','Campinas','Santos','Guarulhos','São Bernardo do Campo','Santo André','Osasco','Ribeirão Preto','Sorocaba','São José dos Campos','Jundiaí','Piracicaba','Bauru','São José do Rio Preto','Mogi das Cruzes','Franca','Taubaté','Limeira','Suzano','Barueri'],
        'RJ': ['Rio de Janeiro','Niterói','Duque de Caxias','Nova Iguaçu','São Gonçalo','Campos dos Goytacazes','Petrópolis','Volta Redonda','Macaé','Nova Friburgo','Angra dos Reis','Cabo Frio','Teresópolis'],
        'MG': ['Belo Horizonte','Uberlândia','Contagem','Juiz de Fora','Betim','Montes Claros','Uberaba','Governador Valadares','Ipatinga','Divinópolis','Poços de Caldas','Varginha'],
        'BA': ['Salvador','Feira de Santana','Vitória da Conquista','Camaçari','Itabuna','Juazeiro','Ilhéus','Lauro de Freitas','Jequié','Porto Seguro'],
        'PR': ['Curitiba','Londrina','Maringá','Ponta Grossa','Cascavel','Foz do Iguaçu','Colombo','Guarapuava','Paranaguá','Toledo'],
        'RS': ['Porto Alegre','Caxias do Sul','Pelotas','Canoas','Santa Maria','Gravataí','Novo Hamburgo','São Leopoldo','Rio Grande','Passo Fundo'],
        'PE': ['Recife','Jaboatão dos Guararapes','Olinda','Caruaru','Paulista','Petrolina','Cabo de Santo Agostinho','Garanhuns'],
        'CE': ['Fortaleza','Caucaia','Juazeiro do Norte','Maracanaú','Sobral','Crato','Itapipoca','Iguatu'],
        'SC': ['Florianópolis','Joinville','Blumenau','São José','Chapecó','Criciúma','Itajaí','Lages','Balneário Camboriú','Tubarão','Palhoça','Brusque','Jaraguá do Sul'],
        'GO': ['Goiânia','Aparecida de Goiânia','Anápolis','Rio Verde','Luziânia','Valparaíso de Goiás','Trindade'],
        'DF': ['Brasília','Taguatinga','Ceilândia','Samambaia','Gama','Sobradinho','Planaltina'],
        'ES': ['Vitória','Vila Velha','Serra','Cariacica','Linhares','Colatina','Guarapari'],
...
App.prototype.getBairrosPorCidade = function(cidade) {
    var bairros = {
        'São Paulo': ['Centro','Pinheiros','Vila Mariana','Moema','Itaim Bibi','Jardins','Tatuapé','Santana','Lapa','Barra Funda','Perdizes','Consolação','Bela Vista','Liberdade','Brooklin','Morumbi','Butantã','Freguesia do Ó','Pirituba','Santo Amaro'],
        'Rio de Janeiro': ['Copacabana','Ipanema','Leblon','Barra da Tijuca','Centro','Botafogo','Flamengo','Laranjeiras','Tijuca','Recreio','Jacarepaguá','Campo Grande','Santa Teresa','Glória','Catete'],
        'Belo Horizonte': ['Savassi','Lourdes','Funcionários','Santo Antônio','Pampulha','Buritis','Cidade Nova','Santa Efigênia','Floresta','Santa Tereza'],
        'Florianópolis': ['Centro','Trindade','Lagoa da Conceição','Ingleses','Canasvieiras','Jurerê','Campeche','Estreito','Coqueiros','Agronômica'],
        'Joinville': ['Centro','América','Glória','Costa e Silva','Boa Vista','Saguaçu','Atiradores','Iririú','Aventureiro','Vila Nova'],
        'Curitiba': ['Centro','Batel','Água Verde','Bigorrilho','Juvevê','Cabral','Portão','Santa Felicidade','Boqueirão','Pinheirinho'],
        'Porto Alegre': ['Moinhos de Vento','Bela Vista','Centro Histórico','Cidade Baixa','Bom Fim','Petrópolis','Menino Deus','Tristeza','Ipanema','Cavalhada'],
        'Salvador': ['Barra','Ondina','Rio Vermelho','Pituba','Itaigara','Costa Azul','Stiep','Amaralina','Brotas','Federação'],
        'Recife': ['Boa Viagem','Pina','Graças','Derby','Casa Forte','Espinheiro','Madalena','Torre','Cordeiro','Santo Amaro'],
        'Fortaleza': ['Meireles','Aldeota','Praia de Iracema','Cocó','Guararapes','Papicu','Dionísio Torres','Fátima','Centro'],
        'Brasília': ['Asa Sul','Asa Norte','Sudoeste','Noroeste','Lago Sul','Lago Norte','Guará','Águas Claras']
    };
...
App.prototype.atualizarCidades = function(cidadeSel) {
    ...
    if (!ee || !ce) return;
    var estado = ee.value, cidades = this.getTodasCidades();
    ce.innerHTML = '<option value="">Selecione a cidade...</option>';
    if (estado && cidades[estado]) {
        for (var i = 0; i < cidades[estado].length; i++) {
            ce.innerHTML += '<option value="' + cidades[estado][i] + '"' + (cidadeSel === cidades[estado][i] ? ' selected' : '') + '>' + cidades[estado][i] + '</option>';
        }
    }
    // Limpar bairros
    var be = document.getElementById('locBairro');
    if (be) be.innerHTML = '<option value="">Selecione o bairro...</option>';
};

App.prototype.atualizarBairros = function(bairroSel) {
    var ce = document.getElementById('locCidade'), be = document.getElementById('locBairro');
    if (!ce || !be) return;
    var cidade = ce.value, bairros = this.getBairrosPorCidade(cidade);
    be.innerHTML = '<option value="">Selecione o bairro...</option>';
    if (bairros) {
        for (var i = 0; i < bairros.length; i++) {
            be.innerHTML += '<option value="' + bairros[i] + '"' + (bairroSel === bairros[i] ? ' selected' : '') + '>' + bairros[i] + '</option>';
        }
    }
};
...
App.prototype.salvarLocalizacao = function() {
    ...
    s.usuarioLogado.localizacao = { estado: estado, cidade: cidade, bairro: bairro };
...
// ===== CONFIGURAÇÕES / QR CODE / EXCLUIR / UPLOAD / EDITAR =====
...
App.prototype.verificarCodigo = function() { this.mostrarToast('Senha redefinida!', 'sucesso'); };
App.prototype.novaObra = function() { this.mostrarToast('Obra criada!', 'sucesso'); };
