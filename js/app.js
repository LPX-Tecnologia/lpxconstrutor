...
var traducoes = {
    pt: {
        ...
        versao: 'Versão do App', idioma: 'Idioma', aparencia: 'Aparência', notificacoes: 'Notificações',
        seguranca: 'Segurança', alterarSenha: 'Alterar Senha', excluirConta: 'Excluir Conta',
        avaliacoes: 'Avaliações', sair: 'SAIR', informacoes: 'Informações', configuracoes: 'Configurações',
        temaClaro: 'Claro', temaEscuro: 'Escuro', portugues: 'Português', espanhol: 'Espanhol', ingles: 'Inglês',
        nome: 'Nome', whatsapp: 'WhatsApp', profissao: 'Profissão', experiencia: 'Experiência',
        ...
        fotoAtualizada: 'Foto atualizada!', temaAlterado: 'Tema:',
        idiomaAlterado: 'Idioma:', empreiteiro: 'Empreiteiro', profissional: 'Profissional',
    ...
    es: {
        ...
        desejaSair: '¿Quieres salir de LPXConstrutor?', simSair: 'SÍ, SALIR', naoSair: 'NO'
    },
    en: {
        ...
        avaliacoes: 'Reviews', sair: 'SIGN OUT', informacoes: 'Information', configuracoes: 'Settings',
        temaClaro: 'Light', temaEscuro: 'Dark', portugues: 'Portuguese', espanhol: 'Spanish', ingles: 'English',
        nome: 'Name', whatsapp: 'WhatsApp', profissao: 'Profession', experiencia: 'Experience',
        ...
        desejaSair: 'Do you want to leave LPXConstrutor?', simSair: 'YES, EXIT', naoSair: 'NO'
    }
};
...
App.prototype.init = function() {
    ...
    if (temaSalvo === 'escuro') { document.body.classList.add('dark-theme'); var ta = document.getElementById('temaAtual'); if (ta) ta.textContent = this.t('temaEscuro'); }
    history.pushState(null, '', window.location.href);
    window.addEventListener('popstate', function() { if (self.telaAtual === 'homeScreen' || self.telaAtual === 'loginScreen') { self.mostrarModalSair(); } else { self.voltarTela(); } });
    authService.onAuthStateChange(function(u) { if (u) { self.usuarioLogado = u; self.atualizarBotaoPublicar(); self.atualizarBotaoObras(); if (self.telaAtual === 'loginScreen' || self.telaAtual === 'cadastroScreen') self.mostrarTela('homeScreen'); } else { self.usuarioLogado = null; self.mostrarTela('loginScreen'); } });
    console.log('✅ App iniciado');
};

App.prototype.atualizarBotaoPublicar = function() { var btn = document.getElementById('btnPublicar'); if (!btn) return; btn.style.display = (this.usuarioLogado && this.usuarioLogado.tipo === 'empreiteiro') ? 'flex' : 'none'; };
App.prototype.atualizarBotaoObras = function() { var btn = document.getElementById('btnObras'); if (!btn) return; btn.style.display = (this.usuarioLogado && this.usuarioLogado.tipo === 'empreiteiro') ? 'flex' : 'none'; };

App.prototype.mostrarTela = function(id) {
    var self = this;
    if (self.telaAtual && self.telaAtual !== id && self.telaAtual !== 'loginScreen') { self.historicoTelas.push(self.telaAtual); }
    document.querySelectorAll('.screen').forEach(function(s) { s.classList.remove('active'); });
    var tela = document.getElementById(id); if (!tela) return; tela.classList.add('active'); self.telaAtual = id;
    var nav = document.getElementById('bottomNav');
    if (nav) { var tn = ['homeScreen','buscaScreen','meuPerfilScreen','chatScreen','publicarVagaScreen','minhasObrasScreen']; nav.style.display = tn.indexOf(id) >= 0 ? 'flex' : 'none'; }
...
App.prototype.voltarTela = function() {
    if (this.historicoTelas.length > 0) { var anterior = this.historicoTelas.pop(); document.querySelectorAll('.screen').forEach(function(s) { s.classList.remove('active'); }); var tela = document.getElementById(anterior); if (tela) { tela.classList.add('active'); this.telaAtual = anterior; } if (anterior === 'meuPerfilScreen') this.carregarMeuPerfil(); if (anterior === 'homeScreen') this.carregarHome(); if (anterior === 'buscaScreen') this.buscarProfissionais(); }
...
App.prototype.selecionarTema = function(tema) {
    var body = document.body; var tc = document.getElementById('temaClaro'), te = document.getElementById('temaEscuro'), ta = document.getElementById('temaAtual');
    if (tema === 'escuro') { body.classList.add('dark-theme'); if (tc) tc.style.background = ''; if (te) te.style.background = '#E0F2FE'; if (ta) ta.textContent = this.t('temaEscuro'); localStorage.setItem('tema', 'escuro'); }
    else { body.classList.remove('dark-theme'); if (tc) tc.style.background = '#E0F2FE'; if (te) te.style.background = ''; if (ta) ta.textContent = this.t('temaClaro'); localStorage.setItem('tema', 'claro'); }
...
App.prototype.fazerLogin = function() {
    var self = this; var e = (document.getElementById('loginEmail')||{}).value||'', s = (document.getElementById('loginSenha')||{}).value||'';
    if (!e || !s) { self.mostrarToast('❌ Preencha todos!', 'erro'); return; }
    authService.login(e, s).then(function(r) { if (r.sucesso) { self.usuarioLogado = r.usuario; self.mostrarToast('✅ Bem-vindo!', 'sucesso'); self.atualizarBotaoPublicar(); self.atualizarBotaoObras(); self.mostrarTela('homeScreen'); } else self.mostrarToast('❌ ' + r.erro, 'erro'); });
};

App.prototype.cadastrar = function() {
    var self = this; var d = { nome: (document.getElementById('cadNome')||{}).value||'', email: (document.getElementById('cadEmail')||{}).value||'', senha: (document.getElementById('cadSenha')||{}).value||'', tipo: (document.getElementById('cadTipo')||{}).value||'profissional', celular: (document.getElementById('cadCelular')||{}).value||'', cpf: ((document.getElementById('cadCPF')||{}).value||'').replace(/\D/g,''), profissao: (document.getElementById('cadProfissao')||{}).value||'', experiencia: (document.getElementById('cadExperiencia')||{}).value||'0', habilidades: (document.getElementById('cadHabilidades')||{}).value||'' };
    if (!d.nome || !d.email || !d.senha) { self.mostrarToast('❌ Preencha todos!', 'erro'); return; }
    authService.cadastrar(d).then(function(r) { if (r.sucesso) { self.usuarioLogado = r.usuario; self.mostrarToast('✅ OK!', 'sucesso'); self.atualizarBotaoPublicar(); self.atualizarBotaoObras(); self.mostrarTela('homeScreen'); } else self.mostrarToast('❌ ' + r.erro, 'erro'); });
};
...
App.prototype.solicitarCodigo = function() { var self=this; var email=document.getElementById('recEmail')?document.getElementById('recEmail').value.trim():''; if(!email||!email.includes('@')){self.mostrarToast('❌ Email inválido!','erro');return;} authService.solicitarCodigoRecuperacao(email).then(function(r){ if(r.sucesso){self.recuperacaoUid=r.uid;self.mostrarToast('✅ Código: '+r.codigo,'sucesso');document.getElementById('recPasso1').style.display='none';document.getElementById('recPasso2').style.display='block';} else self.mostrarToast('❌ '+r.erro,'erro'); }); };
...
App.prototype.carregarHome = function() {
    if(!this.usuarioLogado)return; var h=new Date().getHours(),s='Bom dia';if(h>=12&&h<18)s='Boa tarde';if(h>=18)s='Boa noite';
    ...
    setTimeout(function(){try{if(typeof mapaService!=='undefined')mapaService.initMap()}catch(e){}},500); this.carregarFeed();
};

App.prototype.carregarFeed = function() {
    var self=this,c=document.getElementById('feedContainer');if(!c)return; c.innerHTML='<div class="loading">Carregando...</div>';
    db.collection('vagas').get().then(function(snap){ var vagas=[];snap.forEach(function(doc){var d=doc.data();if(d.ativa!==false)vagas.push({id:doc.id,data:d});});
...
App.prototype.carregarRede = function() {
    ...
    db.collection('conexoes').get().then(function(snap){ var conexoes=[];snap.forEach(function(doc){var d=doc.data();if(d.usuarioId===self.usuarioLogado.id||d.amigoId===self.usuarioLogado.id)conexoes.push({id:doc.id,data:d});});
        ...
        var proms=[];conexoes.forEach(function(con){var aid=con.data.usuarioId===self.usuarioLogado.id?con.data.amigoId:con.data.usuarioId;proms.push(db.collection('usuarios').doc(aid).get().then(function(ud){if(ud.exists)return{usuario:{id:ud.id,...ud.data()},conexao:con.data};return null;}));});
...
// ===== BUSCA (NÃO MOSTRA O PRÓPRIO PERFIL) =====
App.prototype.buscarProfissionais = function() {
    var self = this, c = document.getElementById('buscaResultados');
    if (!c) return;
    ...
    db.collection('usuarios').get().then(function(snap) {
        ...
        var profs = todos.filter(function(u) { return u.data.tipo === 'profissional' && u.data.ativo !== false && u.id !== self.usuarioLogado.id; });
        ...
        if (filtrados.length === 0) { c.innerHTML = '<div class="card" style="text-align:center;padding:40px;"><i class="fas fa-search" style="font-size:60px;color:#ccc;"></i><h3>Nenhum profissional</h3></div>'; return; }
        ...
        filtrados.forEach(function(u) {
            ...
            var msg = self.usuarioLogado && self.usuarioLogado.tipo === 'empreiteiro' ? 'Olá ' + u.data.nome.split(' ')[0] + '! Me chamo ' + self.usuarioLogado.nome + ', sou empreiteiro. Vi seu perfil no LPXConstrutor. Podemos conversar?' : 'Olá ' + u.data.nome.split(' ')[0] + '! Vi seu perfil no LPXConstrutor.';
            html += '<div class="vaga-card"><div class="vaga-header" onclick="window.app.verPerfil(\'' + u.id + '\')"><div class="vaga-avatar">' + (u.data.fotoPerfil ? '<img src="' + u.data.fotoPerfil + '" style="width:100%;height:100%;border-radius:50%;object-fit:cover;">' : '<i class="fas fa-hard-hat"></i>') + '</div><div class="vaga-info"><div class="vaga-nome">' + u.data.nome + '</div><div class="vaga-data"><i class="fas fa-tools"></i> ' + (u.data.profissao || 'Profissional') + ' • <i class="fas fa-calendar"></i> ' + (u.data.experiencia || 0) + ' anos</div><div class="vaga-data"><span style="color:#F47920;">' + '⭐'.repeat(Math.round(sc)) + ' ' + (sc > 0 ? sc.toFixed(1) : 'Novo') + '</span></div></div></div><div class="vaga-footer"><div style="display:flex;gap:6px;flex:1;">' + (w ? '<a href="https://wa.me/55' + w + '?text=' + encodeURIComponent(msg) + '" target="_blank" class="btn btn-success btn-small" style="flex:1;text-decoration:none;display:flex;align-items:center;justify-content:center;gap:4px;" onclick="event.stopPropagation();"><i class="fab fa-whatsapp"></i> WhatsApp</a>' : '') + '<button class="btn btn-primary btn-small" onclick="event.stopPropagation();window.app.iniciarChat(\'' + u.id + '\')" style="flex:1;"><i class="fas fa-comments"></i> Chat</button></div>' + (self.usuarioLogado && self.usuarioLogado.tipo === 'empreiteiro' ? '<button class="btn btn-outline btn-small" onclick="event.stopPropagation();window.app.abrirContratacao(\'' + u.id + '\')" style="background:#1A3A5C;color:white;font-size:11px;"><i class="fas fa-handshake"></i> Contratar</button>' : '') + '<button class="btn btn-outline btn-small" onclick="event.stopPropagation();window.app.adicionarNaRede(\'' + u.id + '\')" style="background:#10B981;color:white;width:36px;height:36px;padding:0;display:flex;align-items:center;justify-content:center;"><i class="fas fa-plus"></i></button></div></div>';
        });
        c.innerHTML = html;
    }).catch(function() { c.innerHTML = '<div class="card">Erro</div>'; });
};
...
App.prototype.verPerfil = function(uid) {
    var self=this; db.collection('usuarios').doc(uid).get().then(function(doc){ if(!doc.exists)return; var u=doc.data(),w=u.celular?u.celular.replace(/\D/g,''):'',c=document.getElementById('perfilPublicoConteudo');if(!c)return;
        ...
        if(self.usuarioLogado&&self.usuarioLogado.id!==uid){ html+='<div style="display:flex;flex-direction:column;gap:10px;margin-top:20px;">'; if(w)html+='<a href="https://wa.me/55'+w+'" target="_blank" class="btn btn-success">WhatsApp</a>'; html+='<button class="btn btn-primary" onclick="window.app.iniciarChat(\''+uid+'\')">💬 Chat</button>'; if(self.usuarioLogado.tipo==='empreiteiro')html+='<button class="btn btn-outline" onclick="window.app.abrirContratacao(\''+uid+'\')" style="background:#1A3A5C;color:white;">🤝 CONTRATAR</button>'; html+='<button class="btn btn-outline" onclick="window.app.verAvaliacoes(\''+uid+'\')">📊 Avaliações</button>'; html+='<button class="btn btn-outline" onclick="window.app.gerarQRCode(\''+uid+'\')">📱 QR Code</button></div>'; }
        c.innerHTML=html;self.mostrarTela('perfilPublicoScreen');
    });
};

// ===== CHAT =====
App.prototype.iniciarChat = function(uid) { var self=this; db.collection('usuarios').doc(uid).get().then(function(doc){if(!doc.exists)return;self.usuarioSelecionado={id:doc.id,data:doc.data()};var h=document.getElementById('chatHeaderInfo');if(h)h.innerHTML='<div style="display:flex;align-items:center;gap:10px;"><div style="width:40px;height:40px;border-radius:50%;background:#F47920;display:flex;align-items:center;justify-content:center;color:white;"><i class="fas fa-user"></i></div><div><strong>'+doc.data().nome+'</strong><div style="font-size:12px;color:#10B981;">Online</div></div></div>';self.carregarMensagens();self.mostrarTela('chatScreen');}); };
App.prototype.carregarMensagens = function() { var self=this,c=document.getElementById('chatMessages');if(!c||!this.usuarioSelecionado)return;c.innerHTML='<div class="loading">Carregando...</div>';db.collection('mensagens').get().then(function(snap){var msgs=[];snap.forEach(function(doc){var d=doc.data();if(d.participantes&&d.participantes.indexOf(self.usuarioLogado.id)>=0&&d.participantes.indexOf(self.usuarioSelecionado.id)>=0)msgs.push({id:doc.id,data:d});});msgs.sort(function(a,b){return(a.data.dataEnvio?a.data.dataEnvio.toDate():new Date(0))-(b.data.dataEnvio?b.data.dataEnvio.toDate():new Date(0));});if(msgs.length===0){c.innerHTML='<div style="text-align:center;padding:60px;color:#999;"><i class="fas fa-comments" style="font-size:60px;"></i><p>Nenhuma mensagem</p></div>';return;}var html='';msgs.forEach(function(m){var isMine=m.data.remetenteId===self.usuarioLogado.id;var hora=m.data.dataEnvio?new Date(m.data.dataEnvio.toDate()).toLocaleTimeString('pt-BR',{hour:'2-digit',minute:'2-digit'}):'';html+='<div class="message '+(isMine?'message-sent':'message-received')+'"><div class="message-content">'+m.data.conteudo+'</div><div class="message-footer"><span class="message-time">'+hora+'</span>'+(isMine?'<span class="message-status">'+(m.data.lida?'✓✓':'✓')+'</span>':'')+'</div></div>';});c.innerHTML=html;setTimeout(function(){c.scrollTop=c.scrollHeight;},100);}); };
App.prototype.enviarMensagem = function() { var self=this,i=document.getElementById('chatInput'),ct=i?i.value.trim():'';console.log('📤 Enviando:',ct);if(!ct){this.mostrarToast('Digite uma mensagem!','erro');return;}if(!this.usuarioSelecionado){this.mostrarToast('Selecione um contato!','erro');return;}if(!this.usuarioLogado){this.mostrarToast('Faça login!','erro');return;}var msg={remetenteId:this.usuarioLogado.id,destinatarioId:this.usuarioSelecionado.id,participantes:[this.usuarioLogado.id,this.usuarioSelecionado.id],conteudo:ct,dataEnvio:firebase.firestore.FieldValue.serverTimestamp(),lida:false};db.collection('mensagens').add(msg).then(function(docRef){console.log('✅ Enviada! ID:',docRef.id);if(i)i.value='';self.carregarMensagens();self.mostrarToast('✅ Enviada!','sucesso');}).catch(function(error){console.error('❌ Erro:',error);self.mostrarToast('❌ Erro ao enviar.','erro');}); };
