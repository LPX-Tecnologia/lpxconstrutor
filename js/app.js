...
App.prototype.mostrarTela = function(id) {
    ...
    if (id === 'chatScreen') {
        if (s.usuarioSelecionado) { s.carregarMensagensLocal(); }
...
App.prototype.carregarListaConversas = function() {
    ...
    s.usuarioSelecionado = null; s.pararListenerChat();
    ...
    s.mostrarTela('chatScreen'); s.buscarTodasConversas();
};

App.prototype.buscarTodasConversas = function() {
    var s = this, container = document.getElementById('listaConversas');
    if (!container) return;
    var conexoes = JSON.parse(localStorage.getItem('conexoesLPX') || '[]'), minhas = [];
    for (var i = 0; i < conexoes.length; i++) { if (conexoes[i].usuarioId === s.usuarioLogado.id || conexoes[i].amigoId === s.usuarioLogado.id) minhas.push(conexoes[i]); }
    var mensagens = JSON.parse(localStorage.getItem('mensagensLPX') || '[]'), conversasMap = {};
    for (var j = 0; j < mensagens.length; j++) { var msg = mensagens[j]; if (msg.remetenteId === s.usuarioLogado.id || msg.destinatarioId === s.usuarioLogado.id) { var outroId = msg.remetenteId === s.usuarioLogado.id ? msg.destinatarioId : msg.remetenteId; if (!conversasMap[outroId]) conversasMap[outroId] = { id: outroId, ultimaMensagem: msg.conteudo, data: msg.dataEnvio || msg.data, naoLidas: 0 }; } }
    for (var k = 0; k < mensagens.length; k++) { if (mensagens[k].destinatarioId === s.usuarioLogado.id && !mensagens[k].lida && conversasMap[mensagens[k].remetenteId]) conversasMap[mensagens[k].remetenteId].naoLidas++; }
    for (var m = 0; m < minhas.length; m++) { var amigoId = minhas[m].usuarioId === s.usuarioLogado.id ? minhas[m].amigoId : minhas[m].usuarioId; if (!conversasMap[amigoId]) conversasMap[amigoId] = { id: amigoId, ultimaMensagem: 'Toque para conversar', data: new Date().toISOString(), naoLidas: 0, semMensagens: true }; }
    var usuarios = JSON.parse(localStorage.getItem('usuariosLPX') || '[]'), ids = Object.keys(conversasMap);
    for (var n = 0; n < ids.length; n++) { var user = usuarios.find(function(u) { return u.id === ids[n]; }); if (user) { conversasMap[ids[n]].nome = user.nome; conversasMap[ids[n]].foto = user.fotoPerfil; } }
    s.renderizarListaConversas(conversasMap, container); s.atualizarBadgeMensagens();
};
...
App.prototype.carregarMensagensLocal = function() {
    var s = this, c = document.getElementById('chatMensagens'); if (!c || !s.usuarioSelecionado) return;
...
App.prototype.enviarMensagem = function() {
    var s = this, input = document.getElementById('chatInput'); if (!input || !s.usuarioSelecionado) return;
    var texto = input.value.trim(); if (!texto) return;
    if (s._enviandoMensagem) return; s._enviandoMensagem = true;
    ...
    s.carregarMensagensLocal(); s.mostrarToast('Enviado!', 'sucesso');
};

App.prototype.iniciarListenerMensagens = function() {
    var s = this; s.pararListenerChat();
    s._intervaloVerificarMsg = setInterval(function() {
        if (!s.usuarioLogado || !s.usuarioSelecionado) return;
        var mensagens = JSON.parse(localStorage.getItem('mensagensLPX') || '[]'), novas = [], alterou = false;
        for (var i = 0; i < mensagens.length; i++) { if (mensagens[i].remetenteId === s.usuarioSelecionado.id && mensagens[i].destinatarioId === s.usuarioLogado.id && !mensagens[i].lida) { novas.push(mensagens[i]); if (s.telaAtual === 'chatScreen') { mensagens[i].lida = true; alterou = true; } } }
        if (alterou) localStorage.setItem('mensagensLPX', JSON.stringify(mensagens));
        if (novas.length > 0) { if (s.telaAtual === 'chatScreen') s.carregarMensagensLocal(); if (s.telaAtual !== 'chatScreen') { s.adicionarNotificacao(s.usuarioLogado.id, '💬 ' + (s.usuarioSelecionado.nome || 'Nova mensagem'), novas[novas.length-1].conteudo.substring(0,50)); s.mostrarToast('💬 Nova mensagem!', 'info'); } s.atualizarBadgeMensagens(); }
    }, 2000);
};
...
App.prototype.marcarMensagensComoLidas = function(remetenteId) { var s = this, mensagens = JSON.parse(localStorage.getItem('mensagensLPX') || '[]'), alterou = false; for (var i = 0; i < mensagens.length; i++) { if (mensagens[i].destinatarioId === s.usuarioLogado.id && mensagens[i].remetenteId === remetenteId && !mensagens[i].lida) { mensagens[i].lida = true; alterou = true; } } if (alterou) { localStorage.setItem('mensagensLPX', JSON.stringify(mensagens)); s.atualizarBadgeMensagens(); } };
App.prototype.atualizarBadgeMensagens = function() { var s = this; if (!s.usuarioLogado) return; var mensagens = JSON.parse(localStorage.getItem('mensagensLPX') || '[]'), naoLidas = 0; for (var i = 0; i < mensagens.length; i++) { if (mensagens[i].destinatarioId === s.usuarioLogado.id && !mensagens[i].lida) naoLidas++; } var badge = document.getElementById('badgeNotificacoes'); if (badge) { badge.textContent = naoLidas > 99 ? '99+' : naoLidas; badge.style.display = naoLidas > 0 ? 'flex' : 'none'; } };

// ===== NOTIFICAÇÕES =====
App.prototype.adicionarNotificacao = function(usuarioId, titulo, mensagem) { var notificacoes = JSON.parse(localStorage.getItem('notificacoesLPX') || '[]'); notificacoes.unshift({ id: 'notif_' + Date.now(), usuarioId: usuarioId, titulo: titulo, mensagem: mensagem, lida: false, data: new Date().toISOString() }); if (notificacoes.length > 100) notificacoes = notificacoes.slice(0, 100); localStorage.setItem('notificacoesLPX', JSON.stringify(notificacoes)); };
App.prototype.mostrarNotificacoes = function() { var s = this; if (!s.usuarioLogado) return; var notificacoes = JSON.parse(localStorage.getItem('notificacoesLPX') || '[]'), minhas = []; for (var i = 0; i < notificacoes.length; i++) { if (notificacoes[i].usuarioId === s.usuarioLogado.id) minhas.push(notificacoes[i]); } for (var i = 0; i < notificacoes.length; i++) { if (notificacoes[i].usuarioId === s.usuarioLogado.id) notificacoes[i].lida = true; } localStorage.setItem('notificacoesLPX', JSON.stringify(notificacoes)); var html = '<div id="modalNotificacoes" style="position:fixed;top:0;left:0;right:0;bottom:0;background:rgba(0,0,0,0.5);z-index:9999;overflow-y:auto;" onclick="if(event.target===this)this.remove()"><div style="background:white;min-height:100vh;max-width:500px;margin:0 auto;"><div style="background:#1A3A5C;color:white;padding:15px;display:flex;justify-content:space-between;"><h3>🔔 Notificações</h3><div><button onclick="window.app.limparNotificacoes()" style="background:rgba(255,255,255,0.2);border:none;color:white;padding:8px 12px;border-radius:8px;cursor:pointer;margin-right:5px;">🗑️</button><button onclick="document.getElementById(\'modalNotificacoes\').remove()" style="background:rgba(255,255,255,0.2);border:none;color:white;padding:8px 15px;border-radius:8px;cursor:pointer;">✕</button></div></div><div style="padding:15px;">'; if (minhas.length === 0) { html += '<div style="text-align:center;padding:40px;"><div style="font-size:50px;">🔔</div><h3>Nenhuma notificação</h3></div>'; } else { for (var i = 0; i < minhas.length; i++) { var n = minhas[i]; html += '<div style="background:' + (n.lida ? '#f9fafb' : '#f0f9ff') + ';border-radius:10px;padding:12px;margin-bottom:8px;border-left:4px solid #1A3A5C;"><div style="font-weight:bold;">' + n.titulo + '</div><div style="font-size:13px;color:#666;">' + n.mensagem + '</div><div style="font-size:10px;color:#999;">' + new Date(n.data).toLocaleString('pt-BR') + '</div></div>'; } } html += '</div></div></div>'; var antigo = document.getElementById('modalNotificacoes'); if (antigo) antigo.remove(); document.body.insertAdjacentHTML('beforeend', html); };
App.prototype.limparNotificacoes = function() { var s = this; if (!confirm('Limpar todas?')) return; var notificacoes = JSON.parse(localStorage.getItem('notificacoesLPX') || '[]'), novas = []; for (var i = 0; i < notificacoes.length; i++) { if (notificacoes[i].usuarioId !== s.usuarioLogado.id) novas.push(notificacoes[i]); } localStorage.setItem('notificacoesLPX', JSON.stringify(novas)); document.getElementById('modalNotificacoes')?.remove(); s.mostrarToast('Limpo!', 'sucesso'); };
...
App.prototype.publicarVagaApp = function() { var s = this; var titulo = (document.getElementById('pubTitulo') || {}).value || '', endereco = (document.getElementById('pubEndereco') || {}).value || '', profissoes = (document.getElementById('pubProfissoes') || {}).value || 'Geral', valor = (document.getElementById('pubValor') || {}).value || ''; if (!titulo || !endereco || !valor) { s.mostrarToast('Preencha todos os campos!', 'erro'); return; } var vaga = { id: 'vaga_' + Date.now(), titulo: titulo, endereco: endereco, profissoes: profissoes, valorHora: parseFloat(valor) || 0, fotoObra: s.vagaFotoBase64 || '', status: 'disponivel', autorId: s.usuarioLogado.id, autorNome: s.usuarioLogado.nome, autorFoto: s.usuarioLogado.fotoPerfil || null, dataCriacao: new Date().toISOString() }; var vagasLocal = JSON.parse(localStorage.getItem('vagasLPX') || '[]'); vagasLocal.unshift(vaga); localStorage.setItem('vagasLPX', JSON.stringify(vagasLocal)); document.getElementById('pubTitulo').value = ''; document.getElementById('pubEndereco').value = ''; document.getElementById('pubProfissoes').value = 'Geral'; document.getElementById('pubValor').value = ''; document.getElementById('pubFotoPreview').src = 'imagem/logo-sem-fundo-lpxconstrutor.png'; s.vagaFotoBase64 = null; s.mostrarToast('Obra publicada! 🏗️', 'sucesso'); setTimeout(function() { s.historicoTelas = []; s.mostrarTela('homeScreen'); s.carregarFeed(); }, 800); };
...
App.prototype.candidatarVaga = function() { this.mostrarToast('✅ Candidatura enviada!', 'sucesso'); };
...
App.prototype.carregarMeuPerfil = function() {
    ...
    var vagas = JSON.parse(localStorage.getItem('vagasLPX') || '[]'), totalObras = vagas.filter(function(v) { return v.autorId === user.id; }).length;
    tela.innerHTML = 
        ...
        '<h2>' + user.nome + '</h2>' +
        '<p style="color:#f0c27f;">' + (user.profissao || user.tipo || 'Usuário') + ' • ⭐ ' + (user.score || 0).toFixed(1) + '</p></div>' +
        ...
        '<div style="display:flex;align-items:center;padding:10px 0;border-bottom:1px solid #f0f0f0;"><i class="fas fa-user" style="width:30px;color:#666;"></i><div style="flex:1;"><div style="font-size:11px;color:#999;">Nome</div><div style="font-weight:500;">' + (user.nome || '') + '</div></div></div>' +
        '<div style="display:flex;align-items:center;padding:10px 0;border-bottom:1px solid #f0f0f0;"><i class="fas fa-envelope" style="width:30px;color:#666;"></i><div style="flex:1;"><div style="font-size:11px;color:#999;">Email</div><div style="font-weight:500;">' + (user.email || '') + '</div></div></div>' +
        '<div style="display:flex;align-items:center;padding:10px 0;border-bottom:1px solid #f0f0f0;"><i class="fas fa-phone" style="width:30px;color:#666;"></i><div style="flex:1;"><div style="font-size:11px;color:#999;">Telefone</div><div style="font-weight:500;">' + (user.celular || '') + '</div></div></div>' +
        '<div style="display:flex;align-items:center;padding:10px 0;border-bottom:1px solid #f0f0f0;"><i class="fas fa-briefcase" style="width:30px;color:#666;"></i><div style="flex:1;"><div style="font-size:11px;color:#999;">Profissão</div><div style="font-weight:500;">' + (user.profissao || '') + '</div></div></div>' +
        ...
        '<button onclick="window.app.gerarQRCodeCompartilhar()" style="width:100%;background:#1A3A5C;color:white;border:none;padding:14px;border-radius:10px;font-weight:bold;cursor:pointer;margin-bottom:8px;"><i class="fas fa-qrcode"></i> Compartilhar Perfil</button>' +
...
App.prototype.atualizarCidades = function(cidadeSel) { var ee = document.getElementById('locEstado'), ce = document.getElementById('locCidade'); if (!ee || !ce) return; var est = ee.value, cid = this.getTodasCidades(); ce.innerHTML = '<option value="">Selecione...</option>'; if (est && cid[est]) for (var i = 0; i < cid[est].length; i++) ce.innerHTML += '<option value="' + cid[est][i] + '"' + (cidadeSel === cid[est][i] ? ' selected' : '') + '>' + cid[est][i] + '</option>'; };
App.prototype.salvarLocalizacao = function() { var s = this; var est = document.getElementById('locEstado')?.value || '', cid = document.getElementById('locCidade')?.value || '', bai = document.getElementById('locBairro')?.value || ''; if (!est || !cid) { s.mostrarToast('Preencha estado e cidade!', 'erro'); return; } s.usuarioLogado.localizacao = { estado: est, cidade: cid, bairro: bai }; localStorage.setItem('usuarioLPX', JSON.stringify(s.usuarioLogado)); document.getElementById('modalLocalizacao')?.remove(); s.mostrarToast('Localização salva!', 'sucesso'); s.carregarMeuPerfil(); };
App.prototype.gerarQRCodeCompartilhar = function() { var s = this; if (!s.usuarioLogado) return; var u = s.usuarioLogado, url = window.location.origin + window.location.pathname + '?perfil=' + u.id; var h = '<div id="modalQRCodeCompartilhar" style="position:fixed;top:0;left:0;right:0;bottom:0;background:rgba(0,0,0,0.7);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px;" onclick="if(event.target===this)window.app.fecharQRCode()"><div style="background:white;border-radius:20px;padding:30px;text-align:center;max-width:350px;" onclick="event.stopPropagation()"><h3>📱 Compartilhar Perfil</h3><div style="margin-bottom:20px;"><div style="width:80px;height:80px;border-radius:50%;overflow:hidden;margin:0 auto 10px;border:3px solid #F47920;">' + (u.fotoPerfil ? '<img src="' + u.fotoPerfil + '" style="width:100%;height:100%;object-fit:cover;">' : '<div style="width:100%;height:100%;background:#1A3A5C;display:flex;align-items:center;justify-content:center;color:white;font-size:35px;">👷</div>') + '</div><p>' + u.nome + '</p></div><div id="qrcodeCompartilharContainer" style="display:flex;justify-content:center;margin-bottom:20px;"></div><button onclick="window.app.fecharQRCode()" style="width:100%;background:#1A3A5C;color:white;border:none;padding:12px;border-radius:10px;cursor:pointer;">FECHAR</button></div></div>'; var a = document.getElementById('modalQRCodeCompartilhar'); if (a) a.remove(); document.body.insertAdjacentHTML('beforeend', h); setTimeout(function() { var c = document.getElementById('qrcodeCompartilharContainer'); if (c && typeof QRCode !== 'undefined') { c.innerHTML = ''; new QRCode(c, { text: url, width: 180, height: 180, colorDark: '#1A3A5C', colorLight: '#ffffff', correctLevel: QRCode.CorrectLevel.M }); } }, 300); };
