...
var App = function() {
    ...
    this.splashMostrada = false; this.init();
};
...
App.prototype.init = function() {
    ...
    // Mostrar splash screen ao iniciar
    s.mostrarSplash();
...
App.prototype.mostrarSplash = function() {
    ...
    if (!splash) {
        ...
        splash.innerHTML = '<img src="imagem/logo-sem-fundo-lpxconstrutor.png" style="width:150px;height:150px;object-fit:contain;animation:float 2s ease-in-out infinite;"><p style="color:white;font-size:24px;font-weight:900;margin-top:20px;">LPXCONSTRUTOR</p><p style="color:#f0c27f;font-size:14px;margin-top:8px;">Rede Profissional da Construção</p><div style="margin-top:30px;"><i class="fas fa-spinner fa-spin" style="color:white;font-size:30px;"></i></div>';
...
App.prototype.mostrarTela = function(id) {
    ...
    if (id === 'recuperarSenhaScreen') {
        var p1 = document.getElementById('recPasso1');
        var p2 = document.getElementById('recPasso2');
...
App.prototype.voltarTela = function() {
    if (this.historicoTelas.length > 0) {
        ...
        if (t) { t.classList.add('active'); this.telaAtual = anterior; }
...
App.prototype.carregarHome = function() {
    ...
    document.getElementById('saudacao').textContent = '👋 ' + sd + ', ' + this.usuarioLogado.nome + '!';
...
App.prototype.carregarRede = function() {
    ...
    db.collection('conexoes').get().then(function(snap) {
        ...
        var proms = [];
        conexoes.forEach(function(con) { var aid = con.data.usuarioId === s.usuarioLogado.id ? con.data.amigoId : con.data.usuarioId; proms.push(db.collection('usuarios').doc(aid).get().then(function(ud) { if (ud.exists) return { usuario: { id: ud.id, ...ud.data() } }; return null; })); });
        Promise.all(proms).then(function(r) {
            var html = '';
            r.forEach(function(x) { if (!x) return; var u = x.usuario; html += '<div class="vaga-card"><div class="vaga-header" onclick="window.app.verPerfil(\'' + u.id + '\')"><div class="vaga-avatar"><i class="fas fa-user"></i></div><div class="vaga-info"><div class="vaga-nome">' + u.nome + '</div></div></div></div>'; });
...
App.prototype.buscarProfissionais = function() {
    ...
    db.collection('usuarios').get().then(function(snap) {
        var todos = [];
        snap.forEach(function(doc) { todos.push({ id: doc.id, data: doc.data() }); });
...
App.prototype.iniciarChat = function(uid) {
    ...
    db.collection('usuarios').doc(uid).get().then(function(doc) {
        if (!doc.exists) return;
        s.usuarioSelecionado = { id: doc.id, data: doc.data() };
...
App.prototype.enviarMensagem = function() {
    var s = this, i = document.getElementById('chatInput'), ct = i ? i.value.trim() : '';
    if (!ct || !s.usuarioSelecionado) return;
    db.collection('mensagens').add({
        ...
        if (i) i.value = '';
        s.mostrarToast('✅ Enviada!', 'sucesso');
    }).catch(function() {
        s.mostrarToast('❌ Erro', 'erro');
    });
};

// ===== PUBLICAR VAGA (CORRIGIDO) =====
App.prototype.abrirTelaPublicacao = function() {
    console.log('📢 abrirTelaPublicacao chamado');
    ...
    // Limpar campos
    var campos = ['vagaTitulo', 'vagaDescricao', 'vagaEndereco'];
    campos.forEach(function(id) {
        var el = document.getElementById(id);
        if (el) el.value = '';
    });
    
    var vh = document.getElementById('vagaValorHora');
    if (vh) vh.value = '';
    
    var cbs = document.querySelectorAll('#profissoesCheckboxes input');
    cbs.forEach(function(cb) { cb.checked = false; });
    
    this.vagaFotoBase64 = null;
    
    console.log('✅ Campos limpos, mostrando tela');
    this.mostrarTela('publicarVagaScreen');
};

App.prototype.publicarVagaApp = function() {
    var s = this;
    console.log('📢 publicarVagaApp chamado');
    ...
    console.log('📝 Dados:', { titulo: t, endereco: e });
    
    if (!t || !e) {
        s.mostrarToast('❌ Preencha título e endereço!', 'erro');
        return;
    }
    ...
    var profissoes = ps.length > 0 ? ps.join(', ') : 'Geral';
    ...
    db.collection('vagas').add(vaga).then(function(docRef) {
        ...
        s.mostrarToast('❌ Erro ao publicar: ' + error.message, 'erro');
    });
};

App.prototype.candidatarVaga = function(vid) {
    var s = this;
    if (!s.usuarioLogado || s.usuarioLogado.tipo !== 'profissional') return;
    db.collection('vagas').doc(vid).get().then(function(doc) {
        if (!doc.exists) return;
        var v = doc.data();
        if (!v.interessados) v.interessados = [];
        if (v.interessados.indexOf(s.usuarioLogado.id) >= 0) return;
        v.interessados.push(s.usuarioLogado.id);
...
App.prototype.abrirContratacao = function(profId) {
    var s = this;
    s.contratarProfId = profId;
    db.collection('usuarios').doc(profId).get().then(function(doc) {
        if (!doc.exists) return;
        document.getElementById('contratarInfo').innerHTML = '<div style="font-size:40px;">👷</div><h3>' + doc.data().nome + '</h3>';
    });
    db.collection('obras').where('usuarioId', '==', s.usuarioLogado.id).where('ativa', '==', true).get().then(function(snap) {
        var sel = document.getElementById('contratarObra');
        sel.innerHTML = '<option value="">Selecione...</option>';
        snap.forEach(function(doc) { sel.innerHTML += '<option value="' + doc.id + '">🏗️ ' + doc.data().nome + '</option>'; });
    });
    s.mostrarTela('contratarScreen');
};

App.prototype.confirmarContratacao = function() {
    var s = this;
    var oid = document.getElementById('contratarObra').value;
    var func = document.getElementById('contratarFuncao').value || '';
    var val = document.getElementById('contratarValor').value || '0';
    if (!oid || !func) return;
    db.collection('conexoes').add({
        usuarioId: s.usuarioLogado.id,
        amigoId: s.contratarProfId,
        obraId: oid,
        funcao: func,
        valorHora: parseFloat(val),
        status: 'contratado',
        dataContratacao: firebase.firestore.FieldValue.serverTimestamp()
    }).then(function() {
        s.mostrarToast('✅ Contratado!', 'sucesso');
        setTimeout(function() { s.voltarTela(); }, 1500);
    });
};

App.prototype.novaObra = function() {
    var n = prompt('Nome da obra:');
    var e = prompt('Endereço:');
    if (!n || !e) return;
    var s = this;
    db.collection('obras').add({
        ...
        ativa: true,
        dataCriacao: firebase.firestore.FieldValue.serverTimestamp()
    }).then(function() { s.carregarMinhasObras(); });
};

App.prototype.carregarMinhasObras = function() {
    var s = this, c = document.getElementById('listaObrasContainer');
    if (!c) return;
    db.collection('obras').where('usuarioId', '==', s.usuarioLogado.id).where('ativa', '==', true).get().then(function(snap) {
        var obras = [];
        snap.forEach(function(doc) { obras.push({ id: doc.id, data: doc.data() }); });
        document.getElementById('totalObras').textContent = obras.length;
        if (obras.length === 0) { c.innerHTML = '<div class="card" style="text-align:center;"><h3>Nenhuma obra</h3></div>'; return; }
        var html = '';
        obras.forEach(function(o) {
            html += '<div class="card" style="cursor:pointer;border-left:4px solid #10B981;" onclick="window.app.verDetalheObra(\'' + o.id + '\')"><h3>🏗️ ' + o.data.nome + '</h3><p>📍 ' + o.data.endereco + '</p></div>';
        });
        c.innerHTML = html;
    });
};
...
App.prototype.verDetalheObra = function(oid) {
    var s = this; s.obraSelecionada = oid;
    var c = document.getElementById('detalheObraConteudo');
    if (!c) return;
    db.collection('obras').doc(oid).get().then(function(doc) {
        var o = doc.data();
        var html = '<div class="card" style="background:linear-gradient(135deg,#1A3A5C,#2C5F8A);color:white;"><h2>🏗️ ' + o.nome + '</h2><p>📍 ' + o.endereco + '</p></div>';
        db.collection('conexoes').where('obraId', '==', oid).get().then(function(snap) {
            if (snap.empty) { html += '<div class="card"><p>Nenhum funcionário</p></div>'; }
            else {
                html += '<h3>👷 Funcionários</h3>';
                snap.forEach(function(doc) {
                    var con = doc.data();
                    html += '<div class="card"><strong>' + (con.funcao || 'Função') + '</strong><br>💰 R$ ' + (con.valorHora || '0') + '/h<br>' + (con.status === 'contratado' ? '<button class="btn btn-danger btn-small" onclick="window.app.demitirFuncionario(\'' + doc.id + '\')">Demitir</button> <button class="btn btn-outline btn-small" onclick="window.app.finalizarContrato(\'' + doc.id + '\')">Finalizar</button>' : '<span>' + con.status + '</span>') + '</div>';
                });
            }
            c.innerHTML = html;
        });
...
App.prototype.demitirFuncionario = function(cid) {
    if (!confirm('Demitir?')) return;
    var s = this;
    db.collection('conexoes').doc(cid).update({ status: 'demitido' }).then(function() { s.verDetalheObra(s.obraSelecionada); });
};

App.prototype.finalizarContrato = function(cid) {
    if (!confirm('Finalizar?')) return;
    var s = this;
    db.collection('conexoes').doc(cid).update({ status: 'finalizado' }).then(function() { s.verDetalheObra(s.obraSelecionada); });
};
...
App.prototype.verAvaliacoes = function(uid) {
    ...
    db.collection('usuarios').doc(uid).get().then(function(doc) {
        if (!doc.exists) return;
        var u = doc.data();
        document.getElementById('avaliacaoScoreGeral').textContent = (u.score || 0).toFixed(1);
        document.getElementById('avaliacaoEstrelas').innerHTML = '⭐'.repeat(Math.round(u.score || 0));
        s.mostrarTela('avaliacoesScreen');
    });
};

App.prototype.abrirDarAvaliacao = function(uid) {
    var s = this; s.avaliarUid = uid;
    db.collection('usuarios').doc(uid).get().then(function(doc) {
        if (!doc.exists) return;
        document.getElementById('avaliarNome').textContent = doc.data().nome;
        s.mostrarTela('darAvaliacaoScreen');
    });
};

App.prototype.setNota = function(n) {
    this.avaliarNota = n;
    document.querySelectorAll('#estrelasAvaliar i').forEach(function(s, i) { s.className = i < n ? 'fas fa-star' : 'far fa-star'; });
};

App.prototype.enviarAvaliacao = function() {
    if (!this.avaliarNota) return;
    var s = this;
    var d = {
        avaliadorId: s.usuarioLogado.id,
        avaliadoId: s.avaliarUid,
        nota: s.avaliarNota,
        positivos: document.getElementById('avalPositivos').value || '',
        dataCriacao: firebase.firestore.FieldValue.serverTimestamp()
    };
    databaseService.avaliarUsuarioCompleto(d).then(function() {
        s.mostrarToast('✅ Avaliação enviada!', 'sucesso');
        setTimeout(function() { s.voltarTela(); }, 1500);
    });
};
...
App.prototype.gerarQRCode = function(uid) {
    ...
    db.collection('usuarios').doc(uid).get().then(function(doc) {
        ...
        var el = document.getElementById('modalQRCode');
...
App.prototype.baixarQRCode = function() {
    ...
    a.download = 'lpxconstrutor-qrcode.png';
    a.href = img.src;
    a.click();
    this.mostrarToast('✅ QR baixado!', 'sucesso');
};
...
App.prototype.salvarPerfil = function() {
    ...
    databaseService.atualizarUsuario(s.usuarioLogado.id, d).then(function() {
        s.usuarioLogado.nome = d.nome;
        s.usuarioLogado.celular = d.celular;
        s.usuarioLogado.habilidades = d.habilidades;
...
App.prototype.uploadFoto = function(e) {
    ...
    r.onload = function(ev) {
        databaseService.atualizarUsuario(s.usuarioLogado.id, { fotoPerfil: ev.target.result }).then(function() {
            ...
            var fp = document.getElementById('fotoPerfilPreview');
            if (fp) fp.src = ev.target.result;
...
App.prototype.confirmarExclusao = function() {
    if (!confirm('⚠️ Tem certeza? Esta ação é IRREVERSÍVEL!')) return;
    this.mostrarToast('📧 Solicitação enviada!', 'sucesso');
    setTimeout(function() { window.app.mostrarTela('loginScreen'); }, 2000);
};
