class App{
    ...
    onTelaCarregada(id){if(id==='homeScreen')this.carregarHome();if(id==='meuPerfilScreen')this.carregarMeuPerfil();if(id==='buscaScreen')this.buscarProfissionais();if(id==='publicarVagaScreen')setTimeout(()=>{const el=document.getElementById('vagaTitulo');if(el)el.focus()},300)}
    mudarTab(tab){
    ...
    async fazerLogin(){
        const e=document.getElementById('loginEmail');const s=document.getElementById('loginSenha');
        const email=e?e.value.trim():'';const senha=s?s.value:'';
        if(!email||!senha){this.mostrarToast('Preencha todos os campos!','erro');return}
    ...
    proximaEtapa(etapa){
        const e1=document.getElementById('etapa1');const e2=document.getElementById('etapa2');
        if(!e1||!e2)return;
        if(etapa===1){e1.style.display='block';e2.style.display='none'}
        else{
            const n=document.getElementById('cadNome');const em=document.getElementById('cadEmail');const se=document.getElementById('cadSenha');
            const nome=n?n.value.trim():'';const email=em?em.value.trim():'';const senha=se?se.value:'';
            if(!nome||nome.length<3){this.mostrarToast('Nome inválido','erro');return}
            if(!email||!email.includes('@')){this.mostrarToast('Email inválido','erro');return}
            if(!senha||senha.length<6){this.mostrarToast('Senha mínima 6 caracteres','erro');return}
            e1.style.display='none';e2.style.display='block'
        }
        window.scrollTo({top:0,behavior:'smooth'})
    }
    toggleProfissao(){const g=document.getElementById('grupoProfissao');const t=document.getElementById('cadTipo');if(g&&t)g.style.display=t.value==='profissional'?'block':'none'}
    async cadastrar(){
        const d={
    ...
    async carregarFeed(){
        ...
        try{
            ...
            if(vagas.length===0){
                c.innerHTML='<div class="card" style="text-align:center;padding:40px;"><i class="fas fa-hard-hat" style="font-size:60px;color:#ccc;"></i><h3>Nenhuma vaga</h3>';
                if(this.usuarioLogado?.tipo==='empreiteiro')c.innerHTML+='<button class="btn btn-primary" onclick="app.abrirTelaPublicacao()" style="margin-top:16px;">PUBLICAR VAGA</button>';
                c.innerHTML+='</div>';return
            }
    ...
    criarVagaCard(v){
        const a=v.autor||{};
        const w=a.celular?a.celular.replace(/\D/g,''):'';
        const d=v.dataCriacao?new Date(v.dataCriacao.toDate()).toLocaleDateString('pt-BR'):'';
        const foto=a.fotoPerfil?'<img src="'+a.fotoPerfil+'" style="width:100%;height:100%;border-radius:50%;object-fit:cover;">':'<i class="fas fa-user-tie"></i>';
        const estrelas='⭐'.repeat(Math.round(a.score||0));
        const botaoQuero=this.usuarioLogado&&this.usuarioLogado.tipo==='profissional'?'<button class="btn btn-primary btn-small" onclick="event.stopPropagation();app.candidatarVaga(\''+v.id+'\')" style="flex:1;">QUERO!</button>':'';
        const botaoWhats=w?'<a href="https://wa.me/55'+w+'" target="_blank" class="btn btn-success btn-small" style="flex:1;text-decoration:none;display:flex;align-items:center;justify-content:center;gap:6px;" onclick="event.stopPropagation();"><i class="fab fa-whatsapp"></i> WhatsApp</a>':'';
        return '<div class="vaga-card"><div class="vaga-header" onclick="app.verPerfil(\''+v.usuarioId+'\')" style="cursor:pointer;"><div class="vaga-avatar">'+foto+'</div><div class="vaga-info"><div class="vaga-nome">'+(a.nome||'Empreiteiro')+'</div><div class="vaga-data">📍 '+(v.endereco||'')+' • '+d+'</div></div><div>'+estrelas+'</div></div><div class="vaga-body"><div class="vaga-titulo">🏗️ '+(v.titulo||'Vaga')+'</div><div class="vaga-descricao">'+(v.descricao||'')+'</div><div class="vaga-tags"><span class="vaga-tag">💰 R$'+v.valorHora+'/h</span><span class="vaga-tag">👷 '+(v.profissoes||'Todas')+'</span></div></div><div class="vaga-footer">'+botaoQuero+botaoWhats+'<button class="btn btn-outline btn-small" onclick="event.stopPropagation();app.verPerfil(\''+v.usuarioId+'\')"><i class="fas fa-user"></i></button></div></div>'
    }
    async verPerfil(uid){
        try{
            ...
            const foto=u.fotoPerfil?'<img src="'+u.fotoPerfil+'" style="width:100%;height:100%;border-radius:50%;object-fit:cover;">':'<i class="fas fa-user" style="font-size:40px;"></i>';
            const bg=u.fotoPerfil?'white':'linear-gradient(135deg, #F47920, #E06B1A)';
            let html='<div class="profile-header-container"><div class="profile-cover"></div><div class="profile-avatar-container"><div class="profile-avatar" style="background:'+bg+';">'+foto+'</div></div></div>';
            html+='<div class="profile-info-card"><h2>'+u.nome+'</h2><p><i class="fas fa-tools"></i> '+(u.profissao||'Profissional')+'</p><p><i class="fas fa-calendar"></i> '+(u.experiencia||0)+' anos</p><div class="stars-container">'+'⭐'.repeat(Math.round(u.score||0))+' '+(u.score?u.score.toFixed(1):'Sem avaliações')+'</div></div>';
            html+='<div class="card"><h3>Habilidades</h3><p>'+(u.habilidades||'Não informado')+'</p></div>';
            html+='<div class="card"><h3>Contato</h3><p>📱 '+(u.celular||'Não informado')+'</p><p>📧 '+u.email+'</p></div>';
            if(this.usuarioLogado&&this.usuarioLogado.id!==u.id){
                ...
                html+='<button class="btn btn-primary" onclick="app.iniciarChat(\''+u.id+'\')"><i class="fas fa-comments"></i> Chat</button>';
                html+='<button class="btn btn-outline" onclick="app.avaliarUsuario(\''+u.id+'\')"><i class="fas fa-star"></i> Avaliar</button>';
                html+='</div>'
            }
            c.innerHTML=html;this.mostrarTela('perfilPublicoScreen')
    ...
    async buscarProfissionais(){
        ...
        try{
            ...
            const profs=todos.filter(u=>(u.tipo==='profissional')&&u.ativo!==false);
            const termo=document.getElementById('buscaInput')?.value?.toLowerCase().trim()||'';
            let filtrados=termo?profs.filter(u=>(u.nome||'').toLowerCase().includes(termo)||(u.profissao||'').toLowerCase().includes(termo)):profs;
            if(filtrados.length===0){
                c.innerHTML='<div class="card" style="text-align:center;padding:40px;"><h3>Nenhum profissional</h3><p style="color:#999;">Total: '+profs.length+' profissionais</p>';
                if(termo)c.innerHTML+='<button class="btn btn-outline btn-small" onclick="document.getElementById(\'buscaInput\').value=\'\';app.buscarProfissionais();">Mostrar todos</button>';
                c.innerHTML+='</div>';return
            }
            c.innerHTML=filtrados.map(u=>{
                const w=u.celular?u.celular.replace(/\D/g,''):'';
                const sc=u.score||0;
                const est='⭐'.repeat(Math.round(sc))||'☆';
                return '<div class="vaga-card" onclick="app.verPerfil(\''+u.id+'\')"><div class="vaga-header"><div class="vaga-avatar"><i class="fas fa-hard-hat"></i></div><div class="vaga-info"><div class="vaga-nome">'+u.nome+'</div><div class="vaga-data">'+(u.profissao||'Profissional')+' • '+(u.experiencia||0)+' anos</div></div><div><div>'+est+'</div><div style="font-size:11px;color:#999;">'+(sc>0?sc.toFixed(1):'Novo')+'</div></div></div><div class="vaga-footer">'+(w?'<a href="https://wa.me/55'+w+'" target="_blank" class="btn btn-success btn-small" style="flex:1;text-decoration:none;display:flex;align-items:center;justify-content:center;gap:6px;" onclick="event.stopPropagation();"><i class="fab fa-whatsapp"></i> WhatsApp</a>':'')+'<button class="btn btn-primary btn-small" onclick="event.stopPropagation();app.iniciarChat(\''+u.id+'\')" style="flex:1;"><i class="fas fa-comments"></i> Chat</button></div></div>'
            }).join('')
    ...
    async carregarRede(){
        ...
        try{
            ...
            c.innerHTML=users.map(u=>{
                const w=u.celular?u.celular.replace(/\D/g,''):'';
                return '<div class="vaga-card" onclick="app.verPerfil(\''+u.id+'\')"><div class="vaga-header"><div class="vaga-avatar"><i class="fas fa-user"></i></div><div class="vaga-info"><div class="vaga-nome">'+u.nome+'</div><div class="vaga-data">'+(u.profissao||'Usuário')+'</div></div></div><div class="vaga-footer">'+(w?'<a href="https://wa.me/55'+w+'" target="_blank" class="btn btn-success btn-small" style="flex:1;text-decoration:none;display:flex;align-items:center;justify-content:center;gap:6px;" onclick="event.stopPropagation();"><i class="fab fa-whatsapp"></i> WhatsApp</a>':'')+'<button class="btn btn-primary btn-small" onclick="event.stopPropagation();app.iniciarChat(\''+u.id+'\')" style="flex:1;"><i class="fas fa-comments"></i> Chat</button></div></div>'
            }).join('')
    ...
    async enviarMensagem(){const i=document.getElementById('chatInput');const ct=i?i.value.trim():'';if(!ct||!this.usuarioSelecionado)return;await db.collection('mensagens').add({remetenteId:this.usuarioLogado.id,destinatarioId:this.usuarioSelecionado.id,participantes:[this.usuarioLogado.id,this.usuarioSelecionado.id],conteudo:ct,dataEnvio:firebase.firestore.FieldValue.serverTimestamp(),lida:false});i.value='';await this.carregarMensagens()}
